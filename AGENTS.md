...
- `D:\Program In My\sub2api` 是本次会话的项目目录；所有项目相关的读取、修改、构建和测试默认都应在该目录内进行。
- 本项目如果需要新增安装依赖，依赖文件统一安装或放置到 `D:\Program Files` 下；不同依赖应分别创建独立文件夹，避免混放。
- 本项目产生的构建产物、临时文件和工具缓存应保存到 `D:\Program In My\sub2api` 下的项目专用缓存目录中，不应写入用户主目录或系统默认缓存目录，除非用户明确要求。

...
func (h *OpenAIGatewayHandler) Images(c *gin.Context) {
	...
	if !compositeOpenAICompatibleTargetAllowed(c, apiKey, requestModel) {
		h.errorResponse(c, http.StatusBadRequest, "invalid_request_error", "Model is not supported by this OpenAI-compatible endpoint for composite groups")
		return
	}
	baseRoutingModel := openAIResolvedRoutingModel(c.Request.Context(), requestModel)
	...
	if err := validateOpenAIImageRoutingModel(requestPlatform, routingModel); err != nil {
		h.errorResponse(c, http.StatusBadRequest, "invalid_request_error", err.Error())
		return
	}
	...
	for {
		...
		result, err := func() (*service.OpenAIForwardResult, error) {
			...
			return h.gatewayService.ForwardImages(requestCtx, c, account, body, parsed, routingModel)
		}()

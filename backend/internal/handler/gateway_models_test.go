...
func TestGatewayModels_Grok45AdvertisesReasoningEffortForGrokBuild(t *testing.T) {
	...
	groupID := int64(4409)
	h := newGatewayModelsHandlerWithPricing(
		[]service.Channel{testPricedChannel(groupID, service.PlatformGrok, "grok-4.5")},
		map[int64]string{groupID: service.PlatformGrok},
	)
...
func TestGatewayModels_IncludeEntryProtocolsReturnsAudioContracts(t *testing.T) {
	...
	customResponse, ok := custom["response_contract"].(map[string]any)
	require.True(t, ok)
...
func TestPublicEntryMetadataForPricedModel_DetectsPublicCapabilities(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "builtin grok text model uses responses entry",
			detail: service.GatewayPricedModelDetail{
				Name:     "grok-4",
				Platform: service.PlatformGrok,
				Pricing: &service.ChannelModelPricing{
					BillingMode: service.BillingModeToken,
					InputPrice:  testPrice(0.000003),
					OutputPrice: testPrice(0.000015),
				},
			},
			wantProtocol:        "openai",
		...
		{
			name: "builtin grok image model uses images entry",
			detail: service.GatewayPricedModelDetail{
		...
		{
			name: "builtin grok video model uses grok generations entry",
			detail: service.GatewayPricedModelDetail{
			...
			wantProtocol:        "openai",
			wantEndpoint:        "/v1/videos/generations",
			wantModelType:       "video",
			wantOperationType:   "video_generation",
			wantExecutionMode:   "async",
			wantSupportsStream:  false,
		...
		{
			name: "grok video custom adapter reuses grok generations entry",
			detail: service.GatewayPricedModelDetail{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got.OperationType == "image_generation" && got.DefaultEntryProtocol == "openai" {
				fields, ok := got.RequestContract["fields"].(map[string]any)
				require.True(t, ok)
				sizeContract, ok := fields["size"].(map[string]any)
				require.True(t, ok)
			...
			if got.OperationType == "image_generation" && got.DefaultEntryProtocol == "gemini" {
				generationConfig, ok := got.RequestContract["generationConfig"].(map[string]any)
				require.True(t, ok)
				imageConfig, ok := generationConfig["imageConfig"].(map[string]any)
				require.True(t, ok)
				aspectRatio, ok := imageConfig["aspectRatio"].(map[string]any)
				require.True(t, ok)
				imageSize, ok := imageConfig["imageSize"].(map[string]any)
				require.True(t, ok)
				require.Contains(t, aspectRatio["values"], "16:9")
				require.Contains(t, imageSize["values"], "4K")
				aspectAliases, ok := aspectRatio["aliases"].(map[string]any)
				require.True(t, ok)
				require.Equal(t, "9:16", aspectAliases["mobile_wallpaper"])

...
func (s *SettingService) workbenchTicketTTLSeconds(settings map[string]string) int {
	...
	if s.cfg != nil && s.cfg.WorkbenchSSO.TicketTTLSeconds > 0 {
		return s.cfg.WorkbenchSSO.TicketTTLSeconds
	}
	return 60
}
...
func (s *SettingService) buildSystemSettingsUpdates(ctx context.Context, settings *SystemSettings) (map[string]string, error) {
	...
	updates[SettingKeyWorkbenchTicketTTLSeconds] = strconv.Itoa(settings.WorkbenchTicketTTLSeconds)
...
func (s *SettingService) InitializeDefaultSettings(ctx context.Context) error {
	...
	defaults := map[string]string{
		...
		SettingKeyWorkbenchSSOEnabled:       strconv.FormatBool(s.cfg != nil && s.cfg.WorkbenchSSO.Enabled),
		SettingKeyWorkbenchBaseURL:          "",
...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	result := &SystemSettings{
		...
		WorkbenchSSOEnabled:              s.workbenchSSOEnabled(settings),
		WorkbenchBaseURL:                 s.workbenchBaseURL(settings),
		WorkbenchTicketTTLSeconds:        s.workbenchTicketTTLSeconds(settings),
		CustomMenuItems:                  settings[SettingKeyCustomMenuItems],
		CustomEndpoints:                  settings[SettingKeyCustomEndpoints],
		BackendModeEnabled:               settings[SettingKeyBackendModeEnabled] == "true",
	}

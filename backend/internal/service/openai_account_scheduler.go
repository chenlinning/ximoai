...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if shouldClearStickySession(account, req.RequestedModel) || !accountMatchesPlatform(account, req.Platform) || (!account.IsOpenAICompatible() && !account.IsOpenAICompatibleCustomAPIKey()) || !account.IsSchedulable() {
		_ = s.service.deleteStickySessionAccountID(ctx, req.GroupID, sessionHash)
		return nil, false, nil
	}
...
func (s *defaultOpenAIAccountScheduler) buildOpenAIAccountLoadPlan(
	...
	if req.RequireCompact {
		...
		for _, candidate := range allCandidates {
			if openAICompactRequiredForAccount(candidate.account, req.RequireCompact) && openAICompactSupportTier(candidate.account) == 0 {
...
func (s *defaultOpenAIAccountScheduler) buildOpenAISelectionOrder(
	...
	if req.RequireCompact {
		...
		for _, candidate := range plan.candidates {
			if !openAICompactRequiredForAccount(candidate.account, req.RequireCompact) {
...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAISelectionOrder(
	...
	for i := 0; i < len(selectionOrder); i++ {
		...
		if openAICompactRequiredForAccount(fresh, req.RequireCompact) && openAICompactSupportTier(fresh) == 0 {
			compactBlocked = true
			continue
		}
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	for i := range accounts {
		...
		if !account.IsSchedulable() || !accountMatchesPlatform(account, req.Platform) || (!account.IsOpenAICompatible() && !account.IsOpenAICompatibleCustomAPIKey()) {
			continue
		}
...
func (s *OpenAIGatewayService) SelectAccountWithScheduler(
	...
	return s.selectAccountWithScheduler(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, "", "", requireCompact, platform, false)
}
...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForImages(
	...
	selection, decision, err := s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, OpenAIUpstreamTransportHTTPSSE, "", requiredCapability, false, platform, false)
	if err == nil && selection != nil && selection.Account != nil {
		return selection, decision, nil
	}
	// 如果要求 native 能力（如指定了模型）但没有可用的 APIKey 账号，回退到 basic（OAuth 账号）
	if requiredCapability == OpenAIImagesCapabilityNative {
		return s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, OpenAIUpstreamTransportHTTPSSE, "", OpenAIImagesCapabilityBasic, false, platform, false)
	}
...
type GatewayOpenAIWSSchedulerScoreWeightsView struct {
	...
	ErrorRate float64
	TTFT      float64
	// Reset prefers accounts whose session window resets sooner; 0 disables it.
	Reset float64
	// QuotaHeadroom prefers accounts with healthier remaining quota; 0 disables it.

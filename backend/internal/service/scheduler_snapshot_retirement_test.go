...
func TestSchedulerFullRebuildCapturesAllRegistryTokensBeforeDBLoad(t *testing.T) {
	...
	require.Equal(t, 2*len(schedulerBucketsForPlatforms(0, builtinSchedulerPlatforms())), captures, "group0 and active-group canonical tokens must be captured before the first DB load")

...
const upstreamPlatformDefinitions: Platform[] = [
...
const ximoAIBuiltinPlatforms: Platform[] = [
...
export const fixedPlatforms: Platform[] = [
  ...CONCRETE_PLATFORM_OPTIONS.map(({ value, label }) => {
    const definition = upstreamPlatformDefinitions.find((platform) => platform.slug === value)
    if (!definition) throw new Error(`Missing fixed platform definition: ${value}`)
    return { ...definition, display_name: label }
  }),
  ...ximoAIBuiltinPlatforms,
]

      ...
      <template #table>
        <DataTable :columns="columns" :data="monitors" :loading="loading">
          ...
          <template #cell-availability_7d="{ row }">
            <span class="text-sm text-accent-900 dark:text-accent-100">{{ formatAvailability(row) }}</span>
          </template>

          <template #cell-latency="{ row }">
            <span class="text-sm text-accent-900 dark:text-accent-100">{{ formatLatency(row.primary_latency_ms) }}</span>
          </template>

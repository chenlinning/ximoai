...
const healthScoreClass = computed(() => {
  if (isSystemIdle.value) return 'text-accent-400'
  const score = healthScoreValue.value
  if (score == null) return 'text-accent-400'
...
const cpuPercentClass = computed(() => {
  ...
  if (v == null) return 'text-accent-900 dark:text-white'
...
const dbMiddleClass = computed(() => {
  ...
  return 'text-accent-900 dark:text-white'
})
...
<template>
  <div :class="['flex flex-col gap-4 rounded-3xl bg-white shadow-sm ring-1 ring-accent-900/5 dark:bg-dark-800 dark:ring-dark-700', props.fullscreen ? 'p-8' : 'p-6']">
    <!-- Top Toolbar -->
    <div class="flex flex-wrap items-center justify-between gap-4 border-b border-accent-100 pb-4 dark:border-dark-700">
      <div>
        <h1 class="flex items-center gap-2 text-xl font-black text-accent-900 dark:text-white">
        ...
        <div v-if="!props.fullscreen" class="mt-1 flex items-center gap-3 text-xs text-accent-500 dark:text-accent-400">
          <span class="flex items-center gap-1.5" :title="props.loading ? t('admin.ops.loadingText') : t('admin.ops.ready')">
            <span class="relative flex h-2 w-2">
              <span class="relative inline-flex h-2 w-2 rounded-full" :class="props.loading ? 'bg-accent-400' : 'bg-green-500'"></span>
            </span>
      ...
      <div class="flex flex-wrap items-center gap-3">
        <template v-if="!props.fullscreen">
          ...
          <div class="mx-1 hidden h-4 w-[1px] bg-accent-200 dark:bg-dark-700 sm:block"></div>
        ...
        <button
          ...
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-lg bg-accent-100 text-accent-500 transition-colors hover:bg-accent-200 dark:bg-dark-700 dark:text-accent-400 dark:hover:bg-dark-600"
          :disabled="loading"
        ...
        <div v-if="!props.fullscreen" class="mx-1 hidden h-4 w-[1px] bg-accent-200 dark:bg-dark-700 sm:block"></div>
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 items-center gap-1.5 rounded-lg bg-accent-100 px-3 text-xs font-bold text-accent-700 transition-colors hover:bg-accent-200 dark:bg-dark-700 dark:text-accent-300 dark:hover:bg-dark-600"
        ...
        <button
          v-if="!props.fullscreen"
          type="button"
          class="flex h-8 w-8 items-center justify-center rounded-lg bg-accent-100 text-accent-700 transition-colors hover:bg-accent-200 dark:bg-dark-700 dark:text-accent-300 dark:hover:bg-dark-600"
    ...
    <div v-if="overview" class="grid grid-cols-1 gap-6 lg:grid-cols-12">
      ...
      <div :class="['rounded-2xl bg-accent-50 dark:bg-dark-900 lg:col-span-5', props.fullscreen ? 'p-6' : 'p-4']">
        <div class="grid h-full grid-cols-1 gap-6 md:grid-cols-[200px_1fr] md:items-center">
          <!-- 1) Health Score -->
          <div
            class="group relative flex cursor-pointer flex-col items-center justify-center rounded-xl py-2 transition-all hover:bg-white/60 dark:hover:bg-dark-800/60 md:border-r md:border-accent-200 md:pr-6 dark:md:border-dark-700"
          >
            ...
            >
              <div class="rounded-xl bg-white p-4 shadow-xl ring-1 ring-black/5 dark:bg-accent-800 dark:ring-white/10">
                <h4 class="mb-3 border-b border-accent-100 pb-2 text-sm font-bold text-accent-900 dark:border-accent-700 dark:text-white flex items-center gap-2">
                ...
                <div class="space-y-3">
                  <div v-for="(item, idx) in diagnosisReport" :key="idx" class="flex gap-3">
                    ...
                    <div class="flex-1">
                      <div class="text-xs font-semibold text-accent-900 dark:text-white">{{ item.message }}</div>
                      <div class="mt-0.5 text-[11px] text-accent-500 dark:text-accent-400">{{ item.impact }}</div>
                ...
                <div class="mt-3 border-t border-accent-100 pt-2 text-[10px] text-accent-400 dark:border-accent-700">
            ...
            <div class="relative flex items-center justify-center">
              <svg :width="circleSize" :height="circleSize" class="-rotate-90 transform">
                <circle
                  ...
                  fill="transparent"
                  class="text-accent-200 dark:text-dark-700"
                  stroke="currentColor"
                />
                <circle
                  :cx="circleSize / 2"
              ...
              <div class="absolute flex flex-col items-center">
                ...
                <span :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase tracking-wider text-accent-400']">{{ t('admin.ops.health') }}</span>
              </div>
            </div>

            <div class="mt-4 text-center" v-if="!props.fullscreen">
              <div class="flex items-center justify-center gap-1 text-xs font-medium text-accent-500">
          ...
          <div class="flex h-full flex-col justify-center py-2">
            <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
              <div class="flex items-center gap-2">
                ...
                <h3 class="text-xs font-bold uppercase tracking-wider text-accent-400">{{ t('admin.ops.realtime.title') }}</h3>
              ...
              <div class="flex flex-wrap gap-1">
                <button
                  ...
                  :class="realtimeWindow === window
                    ...
                    : 'bg-accent-200 text-accent-600 hover:bg-accent-300 dark:bg-dark-700 dark:text-accent-400 dark:hover:bg-dark-600'"
            ...
            <div :class="props.fullscreen ? 'space-y-4' : 'space-y-3'">
              <!-- Row 1: Current -->
              <div>
                <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-accent-400']">{{ t('admin.ops.current') }}</div>
                <div class="mt-1 flex flex-wrap items-baseline gap-x-4 gap-y-2">
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-black text-accent-900 dark:text-white']">{{ displayRealTimeQps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-bold text-accent-500']">QPS</span>
                  </div>
                  <div class="flex items-baseline gap-1.5">
                    <span :class="[props.fullscreen ? 'text-4xl' : 'text-xl sm:text-2xl', 'font-black text-accent-900 dark:text-white']">{{ displayRealTimeTps.toFixed(1) }}</span>
                    <span :class="[props.fullscreen ? 'text-sm' : 'text-xs', 'font-bold text-accent-500']">{{ t('admin.ops.tps') }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <!-- Peak -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-accent-400']">{{ t('admin.ops.peak') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-accent-600 dark:text-accent-400']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-accent-900 dark:text-white">{{ realtimeQpsPeakLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-accent-900 dark:text-white">{{ realtimeTpsPeakLabel }}</span>
                ...
                <!-- Average -->
                <div>
                  <div :class="[props.fullscreen ? 'text-xs' : 'text-[10px]', 'font-bold uppercase text-accent-400']">{{ t('admin.ops.average') }}</div>
                  <div :class="[props.fullscreen ? 'text-base' : 'text-sm', 'mt-1 space-y-0.5 font-medium text-accent-600 dark:text-accent-400']">
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-accent-900 dark:text-white">{{ realtimeQpsAvgLabel }}</span>
                      <span class="text-xs">QPS</span>
                    </div>
                    <div class="flex items-baseline gap-1.5">
                      <span class="font-black text-accent-900 dark:text-white">{{ realtimeTpsAvgLabel }}</span>
                      <span class="text-xs">{{ t('admin.ops.tps') }}</span>
                    </div>
                  </div>
                </div>
      ...
      <div class="grid h-full grid-cols-1 content-center gap-4 sm:grid-cols-2 lg:col-span-7 lg:grid-cols-3">
        <!-- Card 1: Requests -->
        <div class="rounded-2xl bg-accent-50 p-4 dark:bg-dark-900" style="order: 1;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-accent-400">{{ t('admin.ops.requestsTitle') }}</span>
          ...
          <div class="mt-2 space-y-2 text-xs">
            <div class="flex justify-between">
              <span class="text-accent-500">{{ t('admin.ops.requests') }}:</span>
              <span class="font-bold text-accent-900 dark:text-white">{{ totalRequestsLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-accent-500">{{ t('admin.ops.tokens') }}:</span>
              <span class="font-bold text-accent-900 dark:text-white">{{ totalTokensLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-accent-500">{{ t('admin.ops.avgQps') }}:</span>
              <span class="font-bold text-accent-900 dark:text-white">{{ qpsAvgLabel }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-accent-500">{{ t('admin.ops.avgTps') }}:</span>
              <span class="font-bold text-accent-900 dark:text-white">{{ tpsAvgLabel }}</span>
            </div>
          </div>
        </div>

        <!-- Card 2: SLA -->
        <div class="rounded-2xl bg-accent-50 p-4 dark:bg-dark-900" style="order: 2;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              <span class="text-[10px] font-bold uppercase text-accent-400">{{ t('admin.ops.sla') }}</span>
          ...
          <div class="mt-3 h-2 w-full overflow-hidden rounded-full bg-accent-200 dark:bg-dark-700">
          ...
          <div class="mt-3 text-xs">
            <div class="flex justify-between">
              <span class="text-accent-500">{{ t('admin.ops.exceptions') }}:</span>
        ...
        <div class="rounded-2xl bg-accent-50 p-4 dark:bg-dark-900" style="order: 4;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-accent-400">{{ t('admin.ops.latencyDuration') }}</span>
          ...
          <div class="mt-2 flex items-baseline gap-2">
            <div class="text-3xl font-black text-accent-900 dark:text-white">
              {{ durationP99Ms ?? '-' }}
            </div>
            <span class="text-xs font-bold text-accent-400">ms (P99)</span>
          </div>
          <div class="mt-3 grid grid-cols-1 gap-x-3 gap-y-1 text-xs 2xl:grid-cols-2">
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-accent-500">P95:</span>
              <span class="font-bold text-accent-900 dark:text-white">{{ durationP95Ms ?? '-' }}</span>
              <span class="text-accent-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-accent-500">P90:</span>
              <span class="font-bold text-accent-900 dark:text-white">{{ durationP90Ms ?? '-' }}</span>
              <span class="text-accent-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-accent-500">P50:</span>
              <span class="font-bold text-accent-900 dark:text-white">{{ durationP50Ms ?? '-' }}</span>
              <span class="text-accent-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-accent-500">Avg:</span>
              <span class="font-bold text-accent-900 dark:text-white">{{ durationAvgMs ?? '-' }}</span>
              <span class="text-accent-400">ms</span>
            </div>
            <div class="flex items-baseline gap-1 whitespace-nowrap">
              <span class="text-accent-500">Max:</span>
              <span class="font-bold text-accent-900 dark:text-white">{{ durationMaxMs ?? '-' }}</span>
              <span class="text-accent-400">ms</span>
            </div>
          </div>
        </div>
        ...
        <!-- Card 5: TTFT -->
        <div class="rounded-2xl bg-accent-50 p-4 dark:bg-dark-900" style="order: 5;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-accent-400">TTFT</span>
        ...
        <div class="rounded-2xl bg-accent-50 p-4 dark:bg-dark-900" style="order: 3;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-accent-400">{{ t('admin.ops.requestErrors') }}</span>
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              <span class="text-accent-500">{{ t('admin.ops.errorCount') }}:</span>
              <span class="font-bold text-accent-900 dark:text-white">{{ formatNumber(overview.error_count_sla ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-accent-500">{{ t('admin.ops.businessLimited') }}:</span>
              <span class="font-bold text-accent-900 dark:text-white">{{ formatNumber(overview.business_limited_count ?? 0) }}</span>
            </div>
          </div>
        </div>

        <!-- Card 6: Upstream Errors -->
        <div class="rounded-2xl bg-accent-50 p-4 dark:bg-dark-900" style="order: 6;">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-1">
              <span class="text-[10px] font-bold uppercase text-accent-400">{{ t('admin.ops.upstreamErrors') }}</span>
          ...
          <div class="mt-3 space-y-1 text-xs">
            <div class="flex justify-between">
              <span class="text-accent-500">{{ t('admin.ops.errorCountExcl429529') }}:</span>
              <span class="font-bold text-accent-900 dark:text-white">{{ formatNumber(overview.upstream_error_count_excl_429_529 ?? 0) }}</span>
            </div>
            <div class="flex justify-between">
              <span class="text-accent-500">429/529:</span>
              <span class="font-bold text-accent-900 dark:text-white">{{ formatNumber((overview.upstream_429_count ?? 0) + (overview.upstream_529_count ?? 0)) }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="overview" class="mt-2 border-t border-accent-100 pt-4 dark:border-dark-700">
      <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        <!-- CPU -->
        <div class="rounded-xl bg-accent-50 p-3 dark:bg-dark-900">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-accent-400">CPU</div>
          ...
          <div v-if="!props.fullscreen" class="mt-1 text-[10px] text-accent-500 dark:text-accent-400">
        ...
        <div class="rounded-xl bg-accent-50 p-3 dark:bg-dark-900">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-accent-400">{{ t('admin.ops.memory') }}</div>
        ...
        <div class="rounded-xl bg-accent-50 p-3 dark:bg-dark-900">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-accent-400">{{ t('admin.ops.db') }}</div>
        ...
        <div class="rounded-xl bg-accent-50 p-3 dark:bg-dark-900">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-accent-400">Redis</div>
        ...
        <div class="rounded-xl bg-accent-50 p-3 dark:bg-dark-900">
          <div class="flex items-center gap-1">
            <div class="text-[10px] font-bold uppercase tracking-wider text-accent-400">{{ t('admin.ops.goroutines') }}</div>
        ...
        <div class="rounded-xl bg-accent-50 p-3 dark:bg-dark-900">
          <div class="flex items-center justify-between gap-2">
            <div class="flex items-center gap-1">
              <div class="text-[10px] font-bold uppercase tracking-wider text-accent-400">{{ t('admin.ops.jobs') }}</div>
    ...
    <BaseDialog :show="showJobsDetails" :title="t('admin.ops.jobs')" width="wide" @close="showJobsDetails = false">
      <div v-if="!jobHeartbeats.length" class="text-sm text-accent-500 dark:text-accent-400">
        {{ t('admin.ops.noData') }}
      </div>
      <div v-else class="space-y-3">
        <div
          ...
          :key="hb.job_name"
          class="rounded-xl border border-accent-100 bg-white p-4 dark:border-dark-700 dark:bg-dark-900"
        >
          <div class="flex items-center justify-between gap-3">
            <div class="truncate text-sm font-semibold text-accent-900 dark:text-white">{{ hb.job_name }}</div>
            <div class="flex items-center gap-3 text-xs text-accent-500 dark:text-accent-400">
              <span v-if="hb.last_duration_ms != null" class="font-mono">{{ hb.last_duration_ms }}ms</span>
              <span>{{ formatTimeShort(hb.updated_at) }}</span>
            </div>
          </div>

          <div class="mt-2 grid grid-cols-1 gap-2 text-xs text-accent-600 dark:text-accent-300 sm:grid-cols-2">
            <div>
    ...
    <BaseDialog :show="showCustomTimeRangeDialog" :title="t('admin.ops.timeRange.custom')" width="narrow" @close="handleCustomTimeRangeCancel">
      <div class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-accent-700 dark:text-accent-300 mb-1">
            {{ t('admin.ops.customTimeRange.startTime') }}
          </label>
          <input
            v-model="customStartTimeInput"
            type="datetime-local"
            class="w-full rounded-lg border border-accent-300 bg-white px-3 py-2 text-sm text-accent-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-dark-600 dark:bg-dark-800 dark:text-white"
          />
        </div>
        <div>
          <label class="block text-sm font-medium text-accent-700 dark:text-accent-300 mb-1">
        ...
        <div class="flex justify-end gap-3 pt-2">
          <button
            type="button"
            class="rounded-lg bg-accent-100 px-4 py-2 text-sm font-medium text-accent-700 hover:bg-accent-200 dark:bg-dark-700 dark:text-accent-300 dark:hover:bg-dark-600"

...
<template>
  <div class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-accent-900/5 dark:bg-dark-800 dark:ring-dark-700">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        <h3 class="text-sm font-bold text-accent-900 dark:text-white">{{ t('admin.ops.email.title') }}</h3>
        <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">{{ t('admin.ops.email.description') }}</p>
      </div>
    ...
    <div v-if="!config" class="text-sm text-accent-500 dark:text-accent-400">
    ...
    <div v-else class="space-y-6">
      <div class="rounded-2xl bg-accent-50 p-4 dark:bg-dark-700/50">
        <h4 class="mb-2 text-sm font-semibold text-accent-900 dark:text-white">{{ t('admin.ops.email.alertTitle') }}</h4>
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div class="text-xs text-accent-600 dark:text-accent-300">
            {{ t('common.enabled') }}:
            <span class="ml-1 font-medium text-accent-900 dark:text-white">
              {{ config.alert.enabled ? t('common.enabled') : t('common.disabled') }}
            </span>
          </div>
          <div class="text-xs text-accent-600 dark:text-accent-300">
            {{ t('admin.ops.email.recipients') }}:
            <span class="ml-1 font-medium text-accent-900 dark:text-white">{{ config.alert.recipients.length }}</span>
          </div>
          <div class="text-xs text-accent-600 dark:text-accent-300">
          ...
          <div class="text-xs text-accent-600 dark:text-accent-300">
            ...
            <span class="ml-1 font-medium text-accent-900 dark:text-white">{{ config.alert.rate_limit_per_hour }}</span>
          </div>
        </div>
      </div>

      <div class="rounded-2xl bg-accent-50 p-4 dark:bg-dark-700/50">
        <h4 class="mb-2 text-sm font-semibold text-accent-900 dark:text-white">{{ t('admin.ops.email.reportTitle') }}</h4>
        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
          <div class="text-xs text-accent-600 dark:text-accent-300">
            {{ t('common.enabled') }}:
            <span class="ml-1 font-medium text-accent-900 dark:text-white">
          ...
          <div class="text-xs text-accent-600 dark:text-accent-300">
            {{ t('admin.ops.email.recipients') }}:
            <span class="ml-1 font-medium text-accent-900 dark:text-white">{{ config.report.recipients.length }}</span>
          </div>
        </div>
      </div>
    </div>
  ...
  <BaseDialog :show="showEditor" :title="t('admin.ops.email.title')" width="extra-wide" @close="showEditor = false">
    <div v-if="draft" class="space-y-6">
      ...
      <div class="rounded-2xl bg-accent-50 p-4 dark:bg-dark-700/50">
        <h4 class="mb-3 text-sm font-semibold text-accent-900 dark:text-white">{{ t('admin.ops.email.alertTitle') }}</h4>
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div class="mb-1 text-xs font-medium text-accent-600 dark:text-accent-300">{{ t('common.enabled') }}</div>
            <label class="inline-flex items-center gap-2 text-sm text-accent-700 dark:text-accent-300">
              <input v-model="draft.alert.enabled" type="checkbox" class="h-4 w-4 rounded border-accent-300" />
              <span>{{ draft.alert.enabled ? t('common.enabled') : t('common.disabled') }}</span>
            </label>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-accent-600 dark:text-accent-300">{{ t('admin.ops.email.minSeverity') }}</div>
            <Select v-model="draft.alert.min_severity" :options="severityOptions" />
          </div>

          <div class="md:col-span-2">
            <div class="mb-1 text-xs font-medium text-accent-600 dark:text-accent-300">{{ t('admin.ops.email.recipients') }}</div>
            ...
            <div class="mt-1 text-xs text-accent-500 dark:text-accent-400">{{ t('admin.ops.email.recipientsHint') }}</div>
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-accent-600 dark:text-accent-300">{{ t('admin.ops.email.rateLimitPerHour') }}</div>
            <input v-model.number="draft.alert.rate_limit_per_hour" type="number" min="0" max="100000" class="input" />
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-accent-600 dark:text-accent-300">{{ t('admin.ops.email.batchWindowSeconds') }}</div>
            <input v-model.number="draft.alert.batching_window_seconds" type="number" min="0" max="86400" class="input" />
          </div>

          <div>
            <div class="mb-1 text-xs font-medium text-accent-600 dark:text-accent-300">{{ t('admin.ops.email.includeResolved') }}</div>
            <label class="inline-flex items-center gap-2 text-sm text-accent-700 dark:text-accent-300">
              <input v-model="draft.alert.include_resolved_alerts" type="checkbox" class="h-4 w-4 rounded border-accent-300" />
      ...
      <div class="rounded-2xl bg-accent-50 p-4 dark:bg-dark-700/50">
        <h4 class="mb-3 text-sm font-semibold text-accent-900 dark:text-white">{{ t('admin.ops.email.reportTitle') }}</h4>
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            <div class="mb-1 text-xs font-medium text-accent-600 dark:text-accent-300">{{ t('common.enabled') }}</div>
            <label class="inline-flex items-center gap-2 text-sm text-accent-700 dark:text-accent-300">
              <input v-model="draft.report.enabled" type="checkbox" class="h-4 w-4 rounded border-accent-300" />
          ...
          <div class="md:col-span-2">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                <div class="mb-1 text-xs font-medium text-accent-600 dark:text-accent-300">{{ t('admin.ops.email.dailySummary') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-accent-700 dark:text-accent-300">
                    <input v-model="draft.report.daily_summary_enabled" type="checkbox" class="h-4 w-4 rounded border-accent-300" />
                  </label>
                  <input v-model="draft.report.daily_summary_schedule" type="text" class="input" :placeholder="t('admin.ops.email.cronPlaceholder')" />
                </div>
              </div>
              <div>
                <div class="mb-1 text-xs font-medium text-accent-600 dark:text-accent-300">{{ t('admin.ops.email.weeklySummary') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-accent-700 dark:text-accent-300">
                    <input v-model="draft.report.weekly_summary_enabled" type="checkbox" class="h-4 w-4 rounded border-accent-300" />
                  </label>
                  <input v-model="draft.report.weekly_summary_schedule" type="text" class="input" :placeholder="t('admin.ops.email.cronPlaceholder')" />
                </div>
              </div>
              <div>
                <div class="mb-1 text-xs font-medium text-accent-600 dark:text-accent-300">{{ t('admin.ops.email.errorDigest') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-accent-700 dark:text-accent-300">
                    <input v-model="draft.report.error_digest_enabled" type="checkbox" class="h-4 w-4 rounded border-accent-300" />
                  </label>
                  <input v-model="draft.report.error_digest_schedule" type="text" class="input" :placeholder="t('admin.ops.email.cronPlaceholder')" />
                </div>
              </div>
              <div>
                <div class="mb-1 text-xs font-medium text-accent-600 dark:text-accent-300">{{ t('admin.ops.email.errorDigestMinCount') }}</div>
                <input v-model.number="draft.report.error_digest_min_count" type="number" min="0" max="1000000" class="input" />
              </div>
              <div>
                <div class="mb-1 text-xs font-medium text-accent-600 dark:text-accent-300">{{ t('admin.ops.email.accountHealth') }}</div>
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-2 text-sm text-accent-700 dark:text-accent-300">
                    <input v-model="draft.report.account_health_enabled" type="checkbox" class="h-4 w-4 rounded border-accent-300" />
                  </label>
                  <input v-model="draft.report.account_health_schedule" type="text" class="input" :placeholder="t('admin.ops.email.cronPlaceholder')" />
                </div>
              </div>
              <div>
                <div class="mb-1 text-xs font-medium text-accent-600 dark:text-accent-300">{{ t('admin.ops.email.accountHealthThreshold') }}</div>
                <input v-model.number="draft.report.account_health_error_rate_threshold" type="number" min="0" max="100" step="0.1" class="input" />
              </div>
            </div>
            <div class="mt-2 text-xs text-accent-500 dark:text-accent-400">{{ t('admin.ops.email.reportHint') }}</div>
          </div>
        </div>
      </div>
    </div>

...
<template>
  <div class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-accent-900/5 dark:bg-dark-800 dark:ring-dark-700">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        <h3 class="text-sm font-bold text-accent-900 dark:text-white">{{ t('admin.ops.alertRules.title') }}</h3>
        <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">{{ t('admin.ops.alertRules.description') }}</p>
      </div>
    ...
    <div v-if="loading" class="py-10 text-center text-sm text-accent-500 dark:text-accent-400">
      {{ t('admin.ops.alertRules.loading') }}
    </div>

    <div v-else-if="sortedRules.length === 0" class="rounded-xl border border-dashed border-accent-200 p-8 text-center text-sm text-accent-500 dark:border-dark-700 dark:text-accent-400">
      {{ t('admin.ops.alertRules.empty') }}
    </div>

    <div v-else class="max-h-[520px] overflow-hidden rounded-xl border border-accent-200 dark:border-dark-700">
      <div class="max-h-[520px] overflow-y-auto">
        <table class="min-w-full divide-y divide-accent-200 dark:divide-dark-700">
          ...
          <tbody class="divide-y divide-accent-200 bg-white dark:divide-dark-700 dark:bg-dark-800">
            <tr v-for="row in sortedRules" :key="row.id" class="hover:bg-accent-50 dark:hover:bg-dark-700/50">
              <td class="px-4 py-3">
                <div class="text-xs font-bold text-accent-900 dark:text-white">{{ row.name }}</div>
                <div v-if="row.description" class="mt-0.5 line-clamp-2 text-[11px] text-accent-500 dark:text-accent-400">
                  {{ row.description }}
                </div>
                <div v-if="row.updated_at" class="mt-1 text-[10px] text-accent-400">
                  {{ formatDateTime(row.updated_at) }}
                </div>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-accent-700 dark:text-accent-200">
                <span class="font-mono">{{ row.metric_type }}</span>
                <span class="mx-1 text-accent-400">{{ row.operator }}</span>
                <span class="font-mono">{{ row.threshold }}</span>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs font-bold text-accent-700 dark:text-accent-200">
                {{ row.severity }}
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-accent-700 dark:text-accent-200">
      ...
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <div v-if="selectedMetricDefinition" class="mt-1 space-y-0.5 text-xs text-accent-500 dark:text-accent-400">
          ...
          <div class="flex items-center justify-between rounded-xl bg-accent-50 px-4 py-3 dark:bg-dark-800/50 md:col-span-2">
            <span class="text-xs font-bold text-accent-700 dark:text-accent-200">{{ t('admin.ops.alertRules.form.enabled') }}</span>
            <input v-model="draft!.enabled" type="checkbox" class="h-4 w-4 rounded border-accent-300 text-primary-600 focus:ring-primary-500" />
          </div>

          <div class="flex items-center justify-between rounded-xl bg-accent-50 px-4 py-3 dark:bg-dark-800/50 md:col-span-2">
            <span class="text-xs font-bold text-accent-700 dark:text-accent-200">{{ t('admin.ops.alertRules.form.notifyEmail') }}</span>
            <input v-model="draft!.notify_email" type="checkbox" class="h-4 w-4 rounded border-accent-300 text-primary-600 focus:ring-primary-500" />
          </div>
        </div>
      </div>

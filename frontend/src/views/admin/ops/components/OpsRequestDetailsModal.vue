      ...
      <div class="flex h-full min-h-0 flex-col">
        ...
        <div v-if="loading" class="flex flex-1 items-center justify-center py-16">
          <div class="flex flex-col items-center gap-3">
            ...
            <span class="text-sm font-medium text-accent-500 dark:text-accent-400">{{ t('common.loading') }}</span>
          </div>
        </div>

        <!-- Table -->
        <div v-else class="flex min-h-0 flex-1 flex-col">
          <div v-if="items.length === 0" class="rounded-xl border border-dashed border-accent-200 p-10 text-center dark:border-dark-700">
            <div class="text-sm font-medium text-accent-600 dark:text-accent-300">{{ t('admin.ops.requestDetails.empty') }}</div>
            <div class="mt-1 text-xs text-accent-400">{{ t('admin.ops.requestDetails.emptyHint') }}</div>
          </div>
          ...
          <div v-else class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-accent-200 dark:border-dark-700">
            <div class="min-h-0 flex-1 overflow-auto">
              ...
              <tbody class="divide-y divide-accent-200 bg-white dark:divide-dark-700 dark:bg-dark-800">
                <tr v-for="(row, idx) in items" :key="idx" class="hover:bg-accent-50 dark:hover:bg-dark-700/50">
                  ...
                  <td class="whitespace-nowrap px-4 py-3 text-xs font-medium text-accent-700 dark:text-accent-200">
                    {{ (row.platform || 'unknown').toUpperCase() }}
                  </td>
                  <td class="max-w-[240px] truncate px-4 py-3 text-xs text-accent-600 dark:text-accent-300" :title="row.model || ''">
                    {{ row.model || '-' }}
                  </td>
                  <td class="whitespace-nowrap px-4 py-3 text-xs text-accent-600 dark:text-accent-300">
                  ...
                  <td class="px-4 py-3">
                    <div v-if="row.request_id" class="flex items-center gap-2">
                      <span class="max-w-[220px] truncate font-mono text-[11px] text-accent-700 dark:text-accent-200" :title="row.request_id">
                        {{ row.request_id }}
                      </span>
                      <button
                        class="rounded-md bg-accent-100 px-2 py-1 text-[10px] font-bold text-accent-600 hover:bg-accent-200 dark:bg-dark-700 dark:text-accent-300 dark:hover:bg-dark-600"

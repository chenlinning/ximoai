  ...
  <BaseDialog :show="show" :title="title" width="full" :close-on-click-outside="true" @close="close">
    <div v-if="loading" class="flex items-center justify-center py-16">
      <div class="flex flex-col items-center gap-3">
        ...
        <div class="text-sm font-medium text-accent-500 dark:text-accent-400">{{ t('admin.ops.errorDetail.loading') }}</div>
      </div>
    </div>

    <div v-else-if="!detail" class="py-10 text-center text-sm text-accent-500 dark:text-accent-400">
      {{ emptyText }}
    </div>
    ...
    <div v-else class="space-y-6 p-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-xl bg-accent-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-accent-400">{{ t('admin.ops.errorDetail.requestId') }}</div>
          <div class="mt-1 break-all font-mono text-sm font-medium text-accent-900 dark:text-white">
            {{ requestId || '—' }}
          </div>
        </div>

        <div class="rounded-xl bg-accent-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-accent-400">{{ t('admin.ops.errorDetail.time') }}</div>
          <div class="mt-1 text-sm font-medium text-accent-900 dark:text-white">
            {{ formatDateTime(detail.created_at) }}
          </div>
        </div>

        <div class="rounded-xl bg-accent-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-accent-400">
        ...
        <div class="rounded-xl bg-accent-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-accent-400">{{ t('admin.ops.errorDetail.platform') }}</div>
          <div class="mt-1 text-sm font-medium text-accent-900 dark:text-white">
            {{ detail.platform || '—' }}
          </div>
        </div>
        ...
        <div class="rounded-xl bg-accent-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-accent-400">{{ t('admin.ops.errorDetail.group') }}</div>
          <div class="mt-1 text-sm font-medium text-accent-900 dark:text-white">
        ...
        <div class="rounded-xl bg-accent-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-accent-400">{{ t('admin.ops.errorDetail.model') }}</div>
          <div class="mt-1 text-sm font-medium text-accent-900 dark:text-white">
            <template v-if="hasModelMapping(detail)">
              <span class="font-mono">{{ detail.requested_model }}</span>
              <span class="mx-1 text-accent-400">→</span>
        ...
        <div class="rounded-xl bg-accent-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-accent-400">{{ t('admin.ops.errorDetail.inboundEndpoint') }}</div>
          <div class="mt-1 break-all font-mono text-sm font-medium text-accent-900 dark:text-white">
        ...
        <div class="rounded-xl bg-accent-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-accent-400">{{ t('admin.ops.errorDetail.upstreamEndpoint') }}</div>
          <div class="mt-1 break-all font-mono text-sm font-medium text-accent-900 dark:text-white">
        ...
        <div class="rounded-xl bg-accent-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-accent-400">{{ t('admin.ops.errorDetail.status') }}</div>
          <div class="mt-1">
        ...
        <div class="rounded-xl bg-accent-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-accent-400">{{ t('admin.ops.errorDetail.requestType') }}</div>
          <div class="mt-1 text-sm font-medium text-accent-900 dark:text-white">
        ...
        <div class="rounded-xl bg-accent-50 p-4 dark:bg-dark-900">
          <div class="text-xs font-bold uppercase tracking-wider text-accent-400">{{ t('admin.ops.errorDetail.message') }}</div>
          <div class="mt-1 truncate text-sm font-medium text-accent-900 dark:text-white" :title="detail.message">
            {{ detail.message || '—' }}
          </div>
        </div>
      </div>
      ...
      <div class="rounded-xl bg-accent-50 p-6 dark:bg-dark-900">
        <h3 class="text-sm font-black uppercase tracking-wider text-accent-900 dark:text-white">{{ t('admin.ops.errorDetail.responseBody') }}</h3>
        <pre class="mt-4 max-h-[520px] overflow-auto rounded-xl border border-accent-200 bg-white p-4 text-xs text-accent-800 dark:border-dark-700 dark:bg-dark-800 dark:text-accent-100"><code>{{ prettyJSON(primaryResponseBody || '') }}</code></pre>
      </div>

      <!-- Upstream errors list (only for request errors) -->
      <div v-if="showUpstreamList" class="rounded-xl bg-accent-50 p-6 dark:bg-dark-900">
        <div class="flex flex-wrap items-center justify-between gap-2">
          <h3 class="text-sm font-black uppercase tracking-wider text-accent-900 dark:text-white">{{ t('admin.ops.errorDetails.upstreamErrors') }}</h3>
          <div class="text-xs text-accent-500 dark:text-accent-400" v-if="correlatedUpstreamLoading">{{ t('common.loading') }}</div>
        </div>

        <div v-if="!correlatedUpstreamLoading && !correlatedUpstreamErrors.length" class="mt-3 text-sm text-accent-500 dark:text-accent-400">
          {{ t('common.noData') }}
        </div>
        ...
        <div v-else class="mt-4 space-y-3">
          <div
            ...
            :key="ev.id"
            class="rounded-xl border border-accent-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-800"
          >
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div class="text-xs font-black text-accent-900 dark:text-white">
                #{{ idx + 1 }}
                <span v-if="ev.type" class="ml-2 rounded-md bg-accent-100 px-2 py-0.5 font-mono text-[10px] font-bold text-accent-700 dark:bg-dark-700 dark:text-accent-200">{{ ev.type }}</span>
              </div>
              <div class="flex items-center gap-2">
                <div class="font-mono text-xs text-accent-500 dark:text-accent-400">
                  {{ ev.status_code ?? '—' }}
                </div>
                <button
                  type="button"
            ...
            <div class="mt-3 grid grid-cols-1 gap-2 text-xs text-accent-600 dark:text-accent-300 sm:grid-cols-2">
              <div>
                <span class="text-accent-400">{{ t('admin.ops.errorDetail.upstreamEvent.status') }}:</span>
                <span class="ml-1 font-mono">{{ ev.status_code ?? '—' }}</span>
              </div>
              <div>
                <span class="text-accent-400">{{ t('admin.ops.errorDetail.upstreamEvent.requestId') }}:</span>
                <span class="ml-1 font-mono">{{ ev.request_id || ev.client_request_id || '—' }}</span>
              </div>
            </div>

            <div v-if="ev.message" class="mt-3 break-words text-sm font-medium text-accent-900 dark:text-white">{{ ev.message }}</div>

            <pre
              v-if="expandedUpstreamDetailIds.has(ev.id)"
              class="mt-3 max-h-[240px] overflow-auto rounded-xl border border-accent-200 bg-accent-50 p-3 text-xs text-accent-800 dark:border-dark-700 dark:bg-dark-900 dark:text-accent-100"
...
const statusClass = computed(() => {
  ...
  return 'bg-accent-50 text-accent-700 ring-accent-600/20 dark:bg-accent-900/30 dark:text-accent-400 dark:ring-accent-500/30'
})

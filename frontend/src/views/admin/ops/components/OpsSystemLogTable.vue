...
<template>
  <section class="rounded-2xl border border-accent-200 bg-white p-4 shadow-sm dark:border-dark-700 dark:bg-dark-900/60">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div>
        <h3 class="text-sm font-bold text-accent-900 dark:text-white">系统日志</h3>
        <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">默认按最新时间倒序，支持筛选搜索与按条件清理。</p>
      </div>
      <div class="flex flex-wrap items-center gap-2 text-xs">
        <span class="rounded-md bg-accent-100 px-2 py-1 text-accent-700 dark:bg-dark-700 dark:text-accent-200">队列 {{ health.queue_depth }}/{{ health.queue_capacity }}</span>
        <span class="rounded-md bg-accent-100 px-2 py-1 text-accent-700 dark:bg-dark-700 dark:text-accent-200">写入 {{ health.written_count }}</span>
    ...
    <div class="mb-4 rounded-xl border border-accent-200 bg-accent-50 p-3 dark:border-dark-700 dark:bg-dark-800/70">
      <div class="mb-2 flex items-center justify-between">
        <div class="text-xs font-semibold text-accent-700 dark:text-accent-200">运行时日志配置（实时生效）</div>
        <span v-if="runtimeLoading" class="text-xs text-accent-500">加载中...</span>
      </div>
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-6">
        <label class="text-xs text-accent-600 dark:text-accent-300">
          级别
        ...
        <div class="md:col-span-2 xl:col-span-6">
          <div class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            <div class="flex flex-wrap items-center gap-x-4 gap-y-2">
              <label class="inline-flex items-center gap-2 text-xs text-accent-600 dark:text-accent-300">
    ...
    <div class="overflow-hidden rounded-xl border border-accent-200 dark:border-dark-700">
      <div v-if="loading" class="px-4 py-8 text-center text-sm text-accent-500">加载中...</div>
      <div v-else-if="!hasData" class="px-4 py-8 text-center text-sm text-accent-500">暂无系统日志</div>
      <div v-else class="overflow-auto">
        <table class="min-w-full table-fixed divide-y divide-accent-200 dark:divide-dark-700">
          <thead class="bg-accent-50 dark:bg-dark-900">
            <tr>
              <th class="w-[170px] px-3 py-2 text-left text-[11px] font-semibold text-accent-500">时间</th>
              <th class="w-[80px] px-3 py-2 text-left text-[11px] font-semibold text-accent-500">级别</th>
              <th class="px-3 py-2 text-left text-[11px] font-semibold text-accent-500">日志详细信息</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-accent-100 dark:divide-dark-800">
            <tr v-for="row in logs" :key="row.id" class="align-top">
              <td class="px-3 py-2 text-xs text-accent-700 dark:text-accent-300">{{ formatTime(row.created_at) }}</td>
              <td class="px-3 py-2 text-xs">
                <span class="inline-flex rounded-full px-2 py-0.5 font-semibold" :class="levelBadgeClass(row.level)">
                  {{ row.level }}
                </span>
              </td>
              <td class="px-3 py-2 text-xs text-accent-700 dark:text-accent-300 whitespace-normal break-all">

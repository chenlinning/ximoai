...
function severityBadgeClass(severity: string | undefined): string {
  ...
  if (s === 'p3') return 'bg-accent-100 text-accent-700 dark:bg-dark-700 dark:text-accent-300'
  return 'bg-accent-100 text-accent-700 dark:bg-dark-700 dark:text-accent-300'
}
...
function statusBadgeClass(status: string | undefined): string {
  ...
  return 'bg-accent-50 text-accent-700 ring-accent-600/20 dark:bg-accent-900/30 dark:text-accent-300 dark:ring-accent-500/30'
}
...
<template>
  <div class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-accent-900/5 dark:bg-dark-800 dark:ring-dark-700">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        <h3 class="text-sm font-bold text-accent-900 dark:text-white">{{ t('admin.ops.alertEvents.title') }}</h3>
        <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">{{ t('admin.ops.alertEvents.description') }}</p>
      </div>
      ...
      <div class="flex items-center gap-2">
        ...
        <button
          class="flex items-center gap-1.5 rounded-lg bg-accent-100 px-3 py-1.5 text-xs font-bold text-accent-700 transition-colors hover:bg-accent-200 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-dark-700 dark:text-accent-300 dark:hover:bg-dark-600"
          :disabled="loading"
          @click="loadFirstPage"
        >
    ...
    <div v-if="loading" class="flex items-center gap-2 text-sm text-accent-500 dark:text-accent-400">
    ...
    <div v-else-if="empty" class="rounded-xl border border-dashed border-accent-200 p-8 text-center text-sm text-accent-500 dark:border-dark-700 dark:text-accent-400">
      {{ t('admin.ops.alertEvents.empty') }}
    </div>

    <div v-else class="overflow-hidden rounded-xl border border-accent-200 dark:border-dark-700">
      <div class="max-h-[600px] overflow-y-auto" @scroll="onScroll">
        <table class="min-w-full divide-y divide-accent-200 dark:divide-dark-700">
          <thead class="sticky top-0 z-10 bg-accent-50 dark:bg-dark-900">
            <tr>
              <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-accent-500 dark:text-accent-400">
              ...
              <th class="px-4 py-3 text-right text-[11px] font-bold uppercase tracking-wider text-accent-500 dark:text-accent-400">
                {{ t('admin.ops.alertEvents.table.email') }}
              </th>
            </tr>
          </thead>
          <tbody class="divide-y divide-accent-200 bg-white dark:divide-dark-700 dark:bg-dark-800">
            <tr
              v-for="row in events"
              :key="row.id"
              class="cursor-pointer hover:bg-accent-50 dark:hover:bg-dark-700/50"
              @click="openDetail(row)"
              :title="row.title || ''"
            >
              <td class="whitespace-nowrap px-4 py-3 text-xs text-accent-600 dark:text-accent-300">
              ...
              <td class="min-w-[260px] px-4 py-3 text-xs text-accent-700 dark:text-accent-200">
                <div class="font-semibold truncate max-w-[360px]">{{ row.title || '-' }}</div>
                <div v-if="row.description" class="mt-0.5 line-clamp-2 text-[11px] text-accent-500 dark:text-accent-400">
                  {{ row.description }}
                </div>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-accent-600 dark:text-accent-300">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-[11px] text-accent-500 dark:text-accent-400">
              ...
              <td class="whitespace-nowrap px-4 py-3 text-right text-xs">
                ...
                >
                  ...
                  <Icon
                    ...
                    name="ban"
                    size="sm"
                    class="text-accent-400 dark:text-accent-500"
                  />
                  <span class="text-[11px] font-bold text-accent-600 dark:text-accent-300">
        ...
        <div v-if="loadingMore" class="flex items-center justify-center gap-2 py-3 text-xs text-accent-500 dark:text-accent-400">
          <svg class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
        ...
        <div v-else-if="!hasMore && events.length > 0" class="py-3 text-center text-xs text-accent-400">
          -
        </div>
      </div>
    </div>
    ...
    <BaseDialog
      ...
      @close="closeDetail"
    >
      <div v-if="detailLoading" class="flex items-center justify-center py-10 text-sm text-accent-500 dark:text-accent-400">
        {{ t('admin.ops.alertEvents.detail.loading') }}
      </div>

      <div v-else-if="!selected" class="py-10 text-center text-sm text-accent-500 dark:text-accent-400">
        {{ t('admin.ops.alertEvents.detail.empty') }}
      </div>

      <div v-else class="space-y-5">
        <div class="rounded-xl bg-accent-50 p-4 dark:bg-dark-900">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <div class="mt-2 text-sm font-semibold text-accent-900 dark:text-white">
                {{ selected.title || '-' }}
              </div>
              <div v-if="selected.description" class="mt-1 whitespace-pre-wrap text-xs text-accent-600 dark:text-accent-300">
                {{ selected.description }}
              </div>
            </div>

            <div class="flex flex-wrap gap-2">
              <div class="flex items-center gap-2 rounded-lg bg-white px-2 py-1 ring-1 ring-accent-200 dark:bg-dark-800 dark:ring-dark-700">
                <span class="text-[11px] font-bold text-accent-600 dark:text-accent-300">{{ t('admin.ops.alertEvents.detail.silence') }}</span>
                <Select
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="rounded-xl bg-accent-50 p-4 dark:bg-dark-900">
              <div class="text-xs font-bold uppercase tracking-wider text-accent-400">{{ t('admin.ops.alertEvents.detail.firedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-accent-900 dark:text-white">{{ formatDateTime(selected.fired_at || selected.created_at) }}</div>
            </div>
            <div class="rounded-xl bg-accent-50 p-4 dark:bg-dark-900">
              <div class="text-xs font-bold uppercase tracking-wider text-accent-400">{{ t('admin.ops.alertEvents.detail.resolvedAt') }}</div>
              <div class="mt-1 text-sm font-medium text-accent-900 dark:text-white">{{ selected.resolved_at ? formatDateTime(selected.resolved_at) : '-' }}</div>
            </div>
            <div class="rounded-xl bg-accent-50 p-4 dark:bg-dark-900">
              <div class="text-xs font-bold uppercase tracking-wider text-accent-400">{{ t('admin.ops.alertEvents.detail.ruleId') }}</div>
              <div class="mt-1 flex flex-wrap items-center gap-2">
                <div class="font-mono text-sm font-bold text-accent-900 dark:text-white">#{{ selected.rule_id }}</div>
                <a
                  class="inline-flex items-center gap-1 rounded-md bg-white px-2 py-1 text-[11px] font-bold text-accent-700 ring-1 ring-accent-200 hover:bg-accent-50 dark:bg-dark-800 dark:text-accent-200 dark:ring-dark-700 dark:hover:bg-dark-700"
            ...
            <div class="rounded-xl bg-accent-50 p-4 dark:bg-dark-900">
              <div class="text-xs font-bold uppercase tracking-wider text-accent-400">{{ t('admin.ops.alertEvents.detail.dimensions') }}</div>
              <div class="mt-1 text-sm text-accent-900 dark:text-white">
        ...
        <div class="rounded-xl border border-accent-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-800">
          <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
            <div>
              <div class="text-sm font-bold text-accent-900 dark:text-white">{{ t('admin.ops.alertEvents.detail.historyTitle') }}</div>
              <div class="mt-0.5 text-xs text-accent-500 dark:text-accent-400">{{ t('admin.ops.alertEvents.detail.historyHint') }}</div>
            </div>
            <Select :model-value="historyRange" :options="historyRangeOptions" class="w-[140px]" @change="historyRange = String($event || '7d')" />
          </div>

          <div v-if="historyLoading" class="py-6 text-center text-xs text-accent-500 dark:text-accent-400">
            {{ t('admin.ops.alertEvents.detail.historyLoading') }}
          </div>
          <div v-else-if="history.length === 0" class="py-6 text-center text-xs text-accent-500 dark:text-accent-400">
            {{ t('admin.ops.alertEvents.detail.historyEmpty') }}
          </div>
          <div v-else class="overflow-hidden rounded-lg border border-accent-100 dark:border-dark-700">
            <table class="min-w-full divide-y divide-accent-100 dark:divide-dark-700">
              <thead class="bg-accent-50 dark:bg-dark-900">
                <tr>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-accent-500 dark:text-accent-400">{{ t('admin.ops.alertEvents.table.time') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-accent-500 dark:text-accent-400">{{ t('admin.ops.alertEvents.table.status') }}</th>
                  <th class="px-3 py-2 text-left text-[11px] font-bold uppercase tracking-wider text-accent-500 dark:text-accent-400">{{ t('admin.ops.alertEvents.table.metric') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-accent-100 dark:divide-dark-700">
                <tr v-for="it in history" :key="it.id" class="hover:bg-accent-50 dark:hover:bg-dark-700/50">
                  <td class="px-3 py-2 text-xs text-accent-600 dark:text-accent-300">{{ formatDateTime(it.fired_at || it.created_at) }}</td>
                  ...
                  <td class="px-3 py-2 text-xs text-accent-600 dark:text-accent-300">

<template>
  <AppLayout>
      ...
      <template #table>
        ...
        >
          <template #header-select>
            <input
              type="checkbox"
              class="h-4 w-4 cursor-pointer rounded border-accent-300 text-primary-600 focus:ring-primary-500"
          ...
          <template #cell-address="{ row }">
            <div class="flex items-center gap-1.5">
              ...
              <div class="relative">
                <button
                  type="button"
                  class="rounded p-0.5 text-accent-400 hover:text-primary-600 dark:hover:text-primary-400"
                ...
                <div
                  ...
                  class="absolute left-0 top-full z-50 mt-1 w-auto min-w-[180px] rounded-lg border border-accent-200 bg-white py-1 shadow-lg dark:border-dark-500 dark:bg-dark-700"
                >
                  <button
                    v-for="fmt in getCopyFormats(row)"
                    :key="fmt.label"
                    class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-xs hover:bg-accent-100 dark:hover:bg-dark-600"
                    @click.stop="copyFormat(fmt.value)"
                  >
                    <span class="truncate font-mono text-accent-600 dark:text-accent-300">{{ fmt.label }}</span>
                  </button>
                </div>
              </div>
            </div>
          ...
          <template #cell-auth="{ row }">
            <div v-if="row.username || row.password" class="flex items-center gap-1.5">
              <div class="flex flex-col text-xs">
                <span v-if="row.username" class="text-accent-700 dark:text-accent-200">{{ row.username }}</span>
                <span v-if="row.password" class="font-mono text-accent-500 dark:text-accent-400">
              ...
              <button
                v-if="row.password"
                type="button"
                class="ml-1 rounded p-0.5 text-accent-400 hover:text-accent-600 dark:hover:text-accent-300"
          ...
          <template #cell-location="{ row }">
            <div class="flex items-center gap-2">
              ...
              />
              <span v-if="formatLocation(row)" class="text-sm text-accent-700 dark:text-accent-200">
                {{ formatLocation(row) }}
              </span>
              <span v-else class="text-sm text-accent-400">-</span>
            </div>
          ...
          <template #cell-account_count="{ row, value }">
            <button
              v-if="(value || 0) > 0"
              type="button"
              class="inline-flex items-center rounded bg-accent-100 px-2 py-0.5 text-xs font-medium text-primary-700 hover:bg-accent-200 dark:bg-dark-600 dark:text-primary-300 dark:hover:bg-dark-500"
            ...
            <span
              v-else
              class="inline-flex items-center rounded bg-accent-100 px-2 py-0.5 text-xs font-medium text-accent-800 dark:bg-dark-600 dark:text-accent-300"
            >
          ...
          <template #cell-latency="{ row }">
            <div class="flex flex-col gap-1">
              ...
              <div
                ...
                class="flex items-center gap-1 text-xs text-accent-500 dark:text-accent-400"
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-accent-500 transition-colors hover:bg-emerald-50 hover:text-emerald-600 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-emerald-900/20 dark:hover:text-emerald-400"
              >
                <svg
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-accent-500 transition-colors hover:bg-blue-50 hover:text-blue-600 disabled:cursor-not-allowed disabled:opacity-50 dark:hover:bg-blue-900/20 dark:hover:text-blue-400"
              >
                <svg
    ...
    >
      <!-- Tab Switch -->
      <div class="mb-6 flex border-b border-accent-200 dark:border-dark-600">
        <button
          type="button"
          ...
          :class="[
            ...
            createMode === 'standard'
              ...
              : 'border-transparent text-accent-500 hover:text-accent-700 dark:text-accent-400 dark:hover:text-accent-300'
          ]"
        >
      ...
      >
        ...
        <div>
          ...
          <div class="relative">
            ...
            <button
              type="button"
              class="absolute right-3 top-1/2 -translate-y-1/2 text-accent-400 hover:text-accent-600 dark:hover:text-accent-300"
      ...
      <div v-else class="space-y-5">
        ...
        <!-- Parse Result -->
        <div v-if="batchParseResult.total > 0" class="rounded-lg bg-accent-50 p-4 dark:bg-dark-700">
    ...
    >
      <div v-if="qualityReport" class="space-y-4">
        <div class="rounded-lg border border-accent-200 bg-accent-50 p-4 dark:border-dark-600 dark:bg-dark-700">
          <div class="flex items-center justify-between gap-4">
            <div>
              <div class="text-sm text-accent-500 dark:text-accent-400">
                {{ qualityReportProxy?.name || '-' }}
              </div>
              <div class="mt-1 text-sm text-accent-700 dark:text-accent-200">
                {{ qualityReport.summary }}
              </div>
            </div>
            <div class="text-right">
              <div class="text-2xl font-semibold text-accent-900 dark:text-white">
                {{ qualityReport.score }}
              </div>
          ...
          <div class="mt-3 grid grid-cols-2 gap-2 text-xs text-accent-600 dark:text-accent-300">
        ...
        <div class="max-h-80 overflow-auto rounded-lg border border-accent-200 dark:border-dark-600">
          <table class="min-w-full divide-y divide-accent-200 text-sm dark:divide-dark-700">
            <thead class="bg-accent-50 text-xs uppercase text-accent-500 dark:bg-dark-800 dark:text-dark-400">
              <tr>
            ...
            <tbody class="divide-y divide-accent-200 bg-white dark:divide-dark-700 dark:bg-dark-900">
              <tr v-for="item in qualityReport.items" :key="item.target">
                <td class="px-3 py-2 text-accent-900 dark:text-white">{{ qualityTargetLabel(item.target) }}</td>
                <td class="px-3 py-2">
                  <span class="badge" :class="qualityStatusClass(item.status)">{{ qualityStatusLabel(item.status) }}</span>
                </td>
                <td class="px-3 py-2 text-accent-600 dark:text-accent-300">{{ item.http_status ?? '-' }}</td>
                <td class="px-3 py-2 text-accent-600 dark:text-accent-300">
                  {{ typeof item.latency_ms === 'number' ? `${item.latency_ms}ms` : '-' }}
                </td>
                <td class="px-3 py-2 text-accent-600 dark:text-accent-300">
                  <span>{{ item.message || '-' }}</span>
                  <span v-if="item.cf_ray" class="ml-1 text-xs text-accent-400">(cf-ray: {{ item.cf_ray }})</span>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    >
      <div v-if="accountsLoading" class="flex items-center justify-center py-8 text-sm text-accent-500">
        <Icon name="refresh" size="md" class="mr-2 animate-spin" />
        {{ t('common.loading') }}
      </div>
      <div v-else-if="proxyAccounts.length === 0" class="py-6 text-center text-sm text-accent-500">
        ...
        <table class="min-w-full divide-y divide-accent-200 text-sm dark:divide-dark-700">
          ...
          <tbody class="divide-y divide-accent-200 bg-white dark:divide-dark-700 dark:bg-dark-900">
            <tr v-for="account in proxyAccounts" :key="account.id">
              <td class="px-4 py-2 font-medium text-accent-900 dark:text-white">{{ account.name }}</td>
              <td class="px-4 py-2">
                <PlatformTypeBadge :platform="account.platform" :type="account.type" />
              </td>
              <td class="px-4 py-2 text-accent-600 dark:text-accent-300">
                {{ account.notes || '-' }}
              </td>
            </tr>
          </tbody>

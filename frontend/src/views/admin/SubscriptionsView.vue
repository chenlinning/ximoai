  ...
  <AppLayout>
          ...
          <div class="flex flex-1 flex-wrap items-center gap-3">
            ...
            >
              ...
              <button
                ...
                type="button"
                class="absolute right-2 top-1/2 -translate-y-1/2 text-accent-400 hover:text-accent-600 dark:hover:text-accent-300"
              ...
              <div
                ...
                class="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-lg border border-accent-200 bg-white shadow-lg dark:border-accent-700 dark:bg-accent-800"
              >
                <div
                  v-if="filterUserLoading"
                  class="px-4 py-3 text-sm text-accent-500 dark:text-accent-400"
                >
                  {{ t('common.loading') }}
                </div>
                <div
                ...
                <button
                  ...
                  class="w-full px-4 py-2 text-left text-sm hover:bg-accent-100 dark:hover:bg-accent-700"
                >
                  <span class="font-medium text-accent-900 dark:text-white">{{ user.email }}</span>
                  <span class="ml-2 text-accent-500 dark:text-accent-400">#{{ user.id }}</span>
                </button>
              </div>
            </div>
          ...
          <div class="ml-auto flex flex-wrap items-center justify-end gap-3">
            ...
            <div class="relative" ref="columnDropdownRef">
              ...
              <div
                ...
                class="absolute right-0 z-50 mt-2 w-48 origin-top-right rounded-lg border border-accent-200 bg-white shadow-lg dark:border-accent-700 dark:bg-accent-800"
              >
                <div class="p-2">
                  <!-- User column mode selection -->
                  <div class="mb-2 border-b border-accent-200 pb-2 dark:border-accent-700">
                    <div class="px-3 py-1 text-xs font-medium text-accent-500 dark:text-accent-400">
                      {{ t('admin.subscriptions.columns.user') }}
                    </div>
                    <button
                      @click="setUserColumnMode('email')"
                      class="flex w-full items-center justify-between rounded-md px-3 py-2 text-sm text-accent-700 hover:bg-accent-100 dark:text-accent-200 dark:hover:bg-accent-700"
                    >
        ...
        >
            ...
            <div class="min-w-[280px] space-y-2">
              ...
              <div v-if="row.group?.daily_limit_usd" class="usage-row">
                <div class="flex items-center gap-2">
                  ...
                  <div class="h-1.5 flex-1 rounded-full bg-accent-200 dark:bg-dark-600">
                    <div
          ...
          <template #cell-expires_at="{ value }">
            <div v-if="value">
              ...
              <div v-if="getDaysRemaining(value) !== null" class="text-xs text-accent-500">
                {{ getDaysRemaining(value) }} {{ t('admin.subscriptions.daysRemaining') }}
              </div>
            </div>
            <span v-else class="text-sm text-accent-500">{{
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <button
                ...
                class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-accent-500 transition-colors hover:bg-orange-50 hover:text-orange-600 dark:hover:bg-orange-900/20 dark:hover:text-orange-400 disabled:cursor-not-allowed disabled:opacity-50"
              >
        ...
        <div>
          ...
          >
            <template #selected="{ option }">
              ...
              />
              <span v-else class="text-accent-400">{{ t('admin.subscriptions.selectGroup') }}</span>
            </template>
    ...
    >
      <form
        ...
        class="space-y-5"
      >
        <div class="rounded-lg bg-accent-50 p-4 dark:bg-dark-700">
          ...
          <p v-if="extendingSubscription.expires_at" class="mt-1 text-sm text-accent-600 dark:text-accent-400">
      ...
      <transition name="modal">
        <div v-if="showGuideModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @mousedown.self="showGuideModal = false">
          ...
          <div class="relative max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-white p-6 shadow-2xl dark:bg-dark-800">
            <button type="button" class="absolute right-4 top-4 text-accent-400 hover:text-accent-600 dark:hover:text-accent-200" @click="showGuideModal = false">
              <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>

            <h2 class="mb-4 text-lg font-bold text-accent-900 dark:text-white">{{ t('admin.subscriptions.guide.title') }}</h2>
            <p class="mb-5 text-sm text-accent-500 dark:text-accent-400">{{ t('admin.subscriptions.guide.subtitle') }}</p>
            ...
            <div class="mb-5">
              ...
              <div class="ml-8 overflow-hidden rounded-lg border border-accent-200 dark:border-dark-600">
                <table class="w-full text-sm">
                  <tbody>
                    <tr v-for="(row, i) in guideActionRows" :key="i" class="border-b border-accent-100 dark:border-dark-700 last:border-0">
                      <td class="whitespace-nowrap bg-accent-50 px-3 py-2 font-medium text-accent-700 dark:bg-dark-700 dark:text-accent-300">{{ row.action }}</td>
                      <td class="px-3 py-2 text-accent-600 dark:text-accent-400">{{ row.desc }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
...
const getProgressClass = (used: number | null | undefined, limit: number | null): string => {
  if (!limit || limit === 0) return 'bg-accent-400'
  const usedValue = used ?? 0
...
.usage-label {
  @apply w-10 flex-shrink-0 text-xs font-medium text-accent-500 dark:text-accent-400;
}

.usage-amount {
  @apply whitespace-nowrap text-xs tabular-nums text-accent-600 dark:text-accent-300;
}

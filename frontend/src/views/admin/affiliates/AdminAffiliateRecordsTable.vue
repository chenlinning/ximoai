  ...
  <AppLayout>
      ...
      <template #filters>
        <div class="flex flex-wrap items-center gap-3">
          <div class="relative w-full md:w-80">
            <Icon name="search" size="md" class="absolute left-3 top-1/2 -translate-y-1/2 text-accent-400" />
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-aff_code="{ row }">
            <span class="font-mono text-sm text-accent-700 dark:text-accent-300">{{ row.aff_code || '-' }}</span>
          </template>
          <template #cell-order="{ row }">
            <div class="space-y-0.5">
              <div class="font-mono text-sm text-accent-900 dark:text-white">#{{ row.order_id }}</div>
              <div class="max-w-56 truncate text-sm text-accent-500 dark:text-dark-400">{{ row.out_trade_no }}</div>
            </div>
          </template>
          ...
          <template #cell-pay_amount="{ row }">
            <span class="text-sm text-accent-900 dark:text-white">¥{{ formatAmount(row.pay_amount) }}</span>
          </template>
          ...
          <template #cell-created_at="{ row }">
            <span class="text-sm text-accent-700 dark:text-accent-300">{{ formatDateTime(row.created_at) }}</span>
          </template>
        </DataTable>
      </template>
    ...
    >
      ...
      <div v-else-if="selectedOverview" class="space-y-4">
        <div class="rounded-lg border border-accent-100 bg-accent-50 p-4 dark:border-dark-700 dark:bg-dark-800">
          <div class="font-mono text-sm text-accent-900 dark:text-white">#{{ selectedOverview.user_id }}</div>
          <div class="mt-1 text-sm font-medium text-accent-900 dark:text-white">{{ selectedOverview.email || '-' }}</div>
          <div class="mt-0.5 text-sm text-accent-500 dark:text-dark-400">{{ selectedOverview.username || '-' }}</div>
        </div>
...
const UserCell = defineComponent({
  ...
  setup(cellProps, { emit }) {
    return () => h('div', { class: 'space-y-0.5' }, [
      h('div', { class: 'font-mono text-sm text-accent-900 dark:text-white' }, `#${cellProps.id}`),
      h(cellProps.clickable ? 'button' : 'div', {
        class: cellProps.clickable
          ...
          : 'max-w-56 truncate text-sm text-accent-700 dark:text-accent-300',
        type: cellProps.clickable ? 'button' : undefined,
        onClick: cellProps.clickable ? () => emit('open', cellProps.id) : undefined,
      }, cellProps.email || '-'),
      h('div', { class: 'max-w-56 truncate text-sm text-accent-500 dark:text-dark-400' }, cellProps.username || '-'),
    ])
  },
})
  ...
  setup(amountProps) {
    return () => h('span', {
      class: amountProps.strong
        ...
        : 'text-sm text-accent-900 dark:text-white',
  ...
  setup(amountProps) {
    return () => {
      ...
      if (value === null || value === undefined) {
        return h('span', { class: 'text-sm text-accent-400 dark:text-dark-500' }, '-')
      }
...
const OverviewStat = defineComponent({
  ...
  setup(statProps) {
    return () => h('div', { class: 'rounded-lg border border-accent-100 bg-white p-3 dark:border-dark-700 dark:bg-dark-900' }, [
      h('div', { class: 'text-sm text-accent-500 dark:text-dark-400' }, statProps.label),
      h('div', {
        class: statProps.mono
          ? 'mt-1 font-mono text-base font-semibold text-accent-900 dark:text-white'
          : 'mt-1 text-base font-semibold text-accent-900 dark:text-white',
      }, statProps.value),
    ])
  },
})

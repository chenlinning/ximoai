  ...
  <AppLayout>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-description="{ value }">
            <span class="text-sm text-accent-600 dark:text-accent-400">{{ value || '-' }}</span>
          </template>
    ...
    >
      <div class="channel-dialog-body">
        <!-- Tab Bar -->
        <div class="flex items-center border-b border-accent-200 dark:border-dark-700 flex-shrink-0 -mx-4 sm:-mx-6 px-4 sm:px-6 -mt-3 sm:-mt-4">
          <!-- Basic Settings Tab -->
          <button
            type="button"
        ...
        <form id="channel-form" @submit.prevent="handleSubmit" class="flex-1 overflow-y-auto pt-4">
          ...
          <div v-show="activeTab === 'basic'" class="space-y-5">
            ...
            <div>
              ...
              <p class="mt-1 ml-6 text-xs text-accent-400">
              ...
              <div class="flex flex-wrap gap-2">
                <label
                  ...
                  :class="activePlatforms.includes(p)
                    ...
                    : 'border-accent-200 hover:bg-accent-50 dark:border-dark-600 dark:hover:bg-dark-700'"
                >
                  <input
                    type="checkbox"
            ...
            <div class="border-t border-accent-200 pt-4 dark:border-dark-700">
          ...
          >
            ...
            <div>
              <label class="input-label text-xs">
                ...
                <span v-if="section.group_ids.length > 0" class="ml-1 font-normal text-accent-400">
                  ({{ t('admin.channels.form.selectedCount', { count: section.group_ids.length }, `已选 ${section.group_ids.length} 个`) }})
                </span>
              </label>
              <div class="max-h-40 overflow-auto rounded-lg border border-accent-200 bg-accent-50 p-2 dark:border-dark-600 dark:bg-dark-900">
                <div v-if="groupsLoading" class="py-2 text-center text-xs text-accent-500">
                  {{ t('common.loading', 'Loading...') }}
                </div>
                <div v-else-if="getGroupsForPlatform(section.platform).length === 0" class="py-2 text-center text-xs text-accent-500">
                ...
                <div v-else class="flex flex-wrap gap-1">
                  <label
                    ...
                    :key="group.id"
                    class="inline-flex cursor-pointer items-center gap-1.5 rounded-md border border-accent-200 px-2 py-1 text-xs transition-colors hover:bg-accent-50 dark:border-dark-600 dark:hover:bg-dark-700"
                    :class="[
                  ...
                  >
                    <input
                      ...
                      class="h-3 w-3 rounded border-accent-300 text-primary-600 focus:ring-primary-500"
                    ...
                    <span class="text-[10px] text-accent-400">{{ group.account_count || 0 }}</span>
                    <span
            ...
            <div v-if="section.platform === 'anthropic' && webSearchGlobalEnabled" class="border-t border-accent-200 pt-3 dark:border-dark-600">
              <div class="flex items-center justify-between">
                <div>
                  <label class="text-xs font-medium text-accent-700 dark:text-accent-300">
            ...
            <div>
              ...
              <div
                ...
                class="rounded border border-dashed border-accent-300 p-2 text-center text-xs text-accent-400 dark:border-dark-500"
              >
              ...
              <div v-else class="space-y-1">
                ...
                >
                  ...
                  <button
                    ...
                    class="rounded p-0.5 text-accent-400 hover:text-red-500"
                  >
                    <Icon name="trash" size="sm" />
                  </button>
                </div>
            ...
            <div class="mt-4 border-t border-accent-200 pt-4 dark:border-dark-700 space-y-3">
              <div class="flex items-center justify-between">
              ...
              <p
                ...
                class="text-xs italic text-accent-400 dark:text-accent-500"
              >
              ...
              <div
                ...
                :key="ruleIndex"
                class="space-y-3 rounded-lg border border-accent-200 p-4 dark:border-dark-600"
              >
                <div class="flex items-center justify-between">
                  <input
                    v-model="rule.name"
                    :placeholder="t('admin.channels.form.ruleName')"
                    class="bg-transparent text-sm font-medium text-accent-700 placeholder-gray-400 outline-none dark:text-accent-300"
                  />
                ...
                <div>
                  <label class="text-xs text-accent-500 dark:text-accent-400">{{ t('admin.channels.form.ruleGroups') }}</label>
                  <div class="mt-1 flex flex-wrap gap-1">
                    <label
                      ...
                      :class="rule.group_ids.includes(gid)
                        ...
                        : 'border-accent-200 hover:bg-accent-50 dark:border-dark-600 dark:hover:bg-dark-700'"
                    >
                      <input type="checkbox" :checked="rule.group_ids.includes(gid)" class="h-3 w-3 rounded border-accent-300 text-primary-600 focus:ring-primary-500" @change="rule.group_ids.includes(gid) ? rule.group_ids.splice(rule.group_ids.indexOf(gid), 1) : rule.group_ids.push(gid)" />
                      <span :class="['font-medium', platformTextClass(section.platform)]">{{ getGroupNameById(gid) }}</span>
                    </label>
                  </div>
                  <p v-if="section.group_ids.length === 0" class="mt-1 text-xs text-accent-400">
                    {{ t('admin.channels.form.noGroupsInChannel') }}
                  </p>
                </div>

                <div>
                  <label class="text-xs text-accent-500 dark:text-accent-400">{{ t('admin.channels.form.ruleAccounts') }}</label>
                  ...
                  <div class="mt-1 flex flex-wrap gap-1">
                    ...
                    >
                      ...
                      <button type="button" @click="removeRuleAccount(rule, accountId)" class="text-accent-400 hover:text-red-500">
                        <Icon name="x" size="xs" />
                      </button>
                    </span>
                  </div>
                  ...
                  <div class="relative mt-1 rule-account-search-container">
                    ...
                    >
                      ...
                      >
                        ...
                        <span class="ml-2 text-xs text-accent-400">#{{ account.id }}</span>
                      </button>
                    </div>
                  </div>
                  <p class="mt-1 text-xs text-accent-400">
                ...
                <div>
                  <div class="mb-1 flex items-center justify-between">
                    <label class="text-xs text-accent-500 dark:text-accent-400">{{ t('admin.channels.form.ruleModelPricing') }}</label>
                    <button type="button" @click="addRulePricingEntry(sIdx, ruleIndex)" class="text-xs text-primary-600 hover:text-primary-700">
                      + {{ t('common.add') }}
                    </button>
                  </div>
                  <div v-if="rule.pricing.length === 0" class="rounded border border-dashed border-accent-300 p-2 text-center text-xs text-accent-400 dark:border-dark-500">
...
.channel-tab-inactive {
  @apply border-transparent text-accent-500 hover:text-accent-700 hover:border-accent-300 dark:text-accent-400 dark:hover:text-accent-300;
}
</style>

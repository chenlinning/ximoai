<template>
  <AppLayout>
    <div class="mx-auto flex max-w-md flex-col items-center space-y-6 py-8">
      ...
      <p v-if="qrUrl && !expired && scanHint" class="text-center text-sm text-accent-500 dark:text-accent-400">
        {{ scanHint }}
      </p>
      ...
      <div v-else class="text-center">
        <p class="text-sm text-accent-500 dark:text-accent-400">{{ qrUrl ? t('payment.qr.expiresIn') : t('payment.qr.payInNewWindowHint') }}</p>
        <p class="mt-1 text-2xl font-bold tabular-nums text-accent-900 dark:text-white">{{ countdownDisplay }}</p>
        <p class="mt-2 text-sm text-accent-400 dark:text-accent-500">{{ t('payment.qr.waitingPayment') }}</p>
      </div>

<template>
  <component :is="isPopup ? 'div' : AppLayout" :class="isPopup ? 'min-h-screen bg-accent-50 dark:bg-dark-900' : ''">
    <div class="mx-auto max-w-lg space-y-6 py-8" :class="isPopup ? 'px-4' : ''">
      ...
      <div v-else-if="initError" class="card p-8 text-center">
        ...
        <h3 class="text-lg font-semibold text-accent-900 dark:text-white">{{ t('payment.stripeLoadFailed') }}</h3>
        <p class="mt-2 text-sm text-accent-500 dark:text-accent-400">{{ initError }}</p>
        ...
        <template v-if="wechatQrUrl">
          <div class="card p-6">
            <div class="flex flex-col items-center space-y-4">
              <p class="text-lg font-semibold text-accent-900 dark:text-white">{{ t('payment.qr.scanWxpay') }}</p>
              ...
              <p class="text-center text-sm text-accent-500 dark:text-accent-400">{{ t('payment.qr.scanWxpayHint') }}</p>
            </div>
          </div>
          <div class="card p-4 text-center">
            <p class="text-sm text-accent-500 dark:text-accent-400">{{ t('payment.qr.waitingPayment') }}</p>
          </div>
        </template>
        ...
        <template v-else-if="stripeSuccess">
          <div class="card p-6 text-center">
            <div class="flex flex-col items-center gap-3 py-4">
              ...
              <p class="text-sm text-accent-500 dark:text-accent-400">{{ t('payment.stripeSuccessProcessing') }}</p>
            </div>
          </div>
        </template>

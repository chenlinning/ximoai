<template>
  <div>
    ...
    <div v-if="props.loading && !props.stats" class="space-y-0.5">
      ...
      <div class="h-3 w-16 animate-pulse rounded bg-accent-200 dark:bg-accent-700"></div>
    ...
    <div v-else-if="props.stats" class="space-y-0.5 text-xs">
      ...
      <!-- Cost (Account) -->
      <div class="flex items-center gap-1">
        <span class="text-accent-500 dark:text-accent-400">{{ t('usage.accountBilled') }}:</span>
      ...
      <div v-if="props.stats.user_cost != null" class="flex items-center gap-1">
        <span class="text-accent-500 dark:text-accent-400">{{ t('usage.userBilled') }}:</span>
        <span class="font-medium text-accent-700 dark:text-accent-300">{{

<template>
  <div class="flex items-center gap-2">
    ...
    <div v-if="isRateLimited" class="flex flex-col items-center gap-1">
      ...
      <span class="text-[11px] text-accent-400 dark:text-accent-500">{{ rateLimitResumeText }}</span>
    </div>
    ...
    <div v-else-if="isOverloaded" class="flex flex-col items-center gap-1">
      ...
      <span class="text-[11px] text-accent-400 dark:text-accent-500">{{ overloadCountdown }}</span>
    </div>
    ...
    <div v-if="hasError && account.error_message" class="group/error relative">
      ...
      <!-- Tooltip - 向下显示 -->
      <div
        class="invisible absolute left-0 top-full z-[100] mt-1.5 min-w-[200px] max-w-[300px] rounded-lg bg-accent-800 px-3 py-2 text-xs text-white opacity-0 shadow-xl transition-all duration-200 group-hover/error:visible group-hover/error:opacity-100 dark:bg-accent-900"
      >
        <div class="whitespace-pre-wrap break-words leading-relaxed text-accent-300">
          {{ account.error_message }}
        </div>
        <!-- 上方小三角 -->
        <div
          class="absolute bottom-full left-3 border-[6px] border-transparent border-b-accent-800 dark:border-b-accent-900"
        ></div>
      </div>
    </div>
    ...
    <div v-if="isRateLimited" class="group relative">
      ...
      <!-- Tooltip -->
      <div
        class="pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 w-56 -translate-x-1/2 whitespace-normal rounded bg-accent-900 px-3 py-2 text-center text-xs leading-relaxed text-white opacity-0 transition-opacity group-hover:opacity-100 dark:bg-accent-700"
      >
        {{ t('admin.accounts.status.rateLimitedUntil', { time: formatDateTime(account.rate_limit_reset_at) }) }}
        <div
          class="absolute left-1/2 top-full -translate-x-1/2 border-4 border-transparent border-t-accent-900 dark:border-t-accent-700"
        ></div>
      </div>
    </div>

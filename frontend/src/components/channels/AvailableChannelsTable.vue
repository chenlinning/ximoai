<template>
  <div class="card overflow-hidden">
    <table class="w-full table-fixed border-collapse text-sm">
      <thead>
        <tr class="border-b border-accent-100 bg-accent-50/50 text-xs font-medium uppercase tracking-wide text-accent-500 dark:border-dark-700 dark:bg-dark-800/50 dark:text-accent-400">
      ...
      <tbody v-if="loading">
        <tr>
          <td colspan="5" class="py-10 text-center">
            <Icon name="refresh" size="lg" class="inline-block animate-spin text-accent-400" />
          </td>
        </tr>
      </tbody>
      <tbody v-else-if="rows.length === 0">
        <tr>
          <td colspan="5" class="py-12 text-center">
            <Icon name="inbox" size="xl" class="mx-auto mb-3 h-12 w-12 text-accent-400" />
            <p class="text-sm text-accent-500 dark:text-accent-400">{{ emptyLabel }}</p>
          </td>
        </tr>
      </tbody>
      ...
      <tbody
        ...
        class="border-b-2 border-accent-200 last:border-b-0 dark:border-dark-600"
      >
        <tr
          v-for="(section, secIdx) in channel.platforms"
          :key="`${channel.name}-${section.platform}`"
          class="transition-colors hover:bg-accent-50/40 dark:hover:bg-dark-800/40"
          :class="{ 'border-t border-accent-100/70 dark:border-dark-700/50': secIdx > 0 }"
        >
          <!-- 渠道名：只在第一行渲染并用 rowspan 纵向合并 -->
          <td
            v-if="secIdx === 0"
            :rowspan="channel.platforms.length"
            class="px-4 py-3 text-center align-middle font-medium text-accent-900 dark:text-white"
          >
            {{ channel.name }}
          </td>
          ...
          <td
            v-if="secIdx === 0"
            :rowspan="channel.platforms.length"
            class="px-4 py-3 align-middle text-xs text-accent-500 dark:text-accent-400"
          >
          ...
          <td class="align-top px-4 py-3">
            <div class="flex flex-col gap-1.5">
              ...
              >
                <span
                  class="inline-flex items-center gap-0.5 text-[10px] font-medium uppercase text-accent-500 dark:text-accent-400"
              ...
              <span v-if="section.groups.length === 0" class="text-xs text-accent-400">-</span>
            </div>
          </td>
          ...
          <td class="align-top px-4 py-3">
            <div class="flex flex-wrap gap-1">
              ...
              />
              <span v-if="section.supported_models.length === 0" class="text-xs text-accent-400">
                {{ noModelsLabel }}
              </span>
            </div>
          </td>

...
const statusMap: Record<OrderStatus, { key: string; class: string }> = {
  ...
  EXPIRED: { key: 'payment.status.expired', class: 'bg-accent-100 text-accent-800 dark:bg-accent-900/30 dark:text-accent-400' },
  CANCELLED: { key: 'payment.status.cancelled', class: 'bg-accent-100 text-accent-800 dark:bg-accent-900/30 dark:text-accent-400' },
...
const statusClass = computed(() => {
  const entry = statusMap[props.status]
  return entry?.class ?? 'bg-accent-100 text-accent-800 dark:bg-accent-900/30 dark:text-accent-400'
})
</script>

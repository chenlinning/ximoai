<template>
  <div class="space-y-4">
    ...
    <template v-if="outcome === 'success'">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <p class="text-lg font-bold text-accent-900 dark:text-white">{{ props.orderType === 'subscription' ? t('payment.result.subscriptionSuccess') : t('payment.result.success') }}</p>
          <div v-if="paidOrder" class="w-full rounded-xl bg-accent-50 p-4 dark:bg-dark-800">
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                <span class="text-accent-500 dark:text-accent-400">{{ t('payment.orders.orderId') }}</span>
                <span class="font-medium text-accent-900 dark:text-white">#{{ paidOrder.id }}</span>
              </div>
              <div v-if="paidOrder.out_trade_no" class="flex justify-between">
                ...
                <span class="font-medium text-accent-900 dark:text-white">{{ paidOrder.out_trade_no }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-accent-500 dark:text-accent-400">{{ t('payment.orders.amount') }}</span>
                <span class="font-medium text-accent-900 dark:text-white">{{ paidOrder.order_type === 'balance' ? '$' : '¥' }}{{ paidOrder.amount.toFixed(2) }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-accent-500 dark:text-accent-400">{{ t('payment.orders.payAmount') }}</span>
                <span class="font-medium text-accent-900 dark:text-white">¥{{ paidOrder.pay_amount.toFixed(2) }}</span>
              </div>
            </div>
          </div>
    ...
    <template v-else-if="outcome === 'cancelled'">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          <div class="flex h-16 w-16 items-center justify-center rounded-full bg-accent-100 dark:bg-dark-700">
            <svg class="h-8 w-8 text-accent-400 dark:text-accent-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </div>
          <p class="text-lg font-bold text-accent-900 dark:text-white">{{ t('payment.qr.cancelled') }}</p>
          <p class="text-sm text-accent-500 dark:text-accent-400">{{ t('payment.qr.cancelledDesc') }}</p>
          <button class="btn btn-primary" @click="handleDone">{{ t('common.confirm') }}</button>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="outcome === 'expired'">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <p class="text-lg font-bold text-accent-900 dark:text-white">{{ t('payment.qr.expired') }}</p>
          <p class="text-sm text-accent-500 dark:text-accent-400">{{ t('payment.qr.expiredDesc') }}</p>
          <button class="btn btn-primary" @click="handleDone">{{ t('common.confirm') }}</button>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="qrUrl">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4">
          <p class="text-lg font-semibold text-accent-900 dark:text-white">{{ scanTitle }}</p>
          ...
          <p v-if="scanHint" class="text-center text-sm text-accent-500 dark:text-accent-400">{{ scanHint }}</p>
      ...
      <div class="card p-4 text-center">
        <p class="text-sm text-accent-500 dark:text-accent-400">{{ t('payment.qr.expiresIn') }}</p>
        <p class="mt-1 text-2xl font-bold tabular-nums text-accent-900 dark:text-white">{{ countdownDisplay }}</p>
        <p class="mt-1 text-xs text-accent-400 dark:text-accent-500">{{ t('payment.qr.waitingPayment') }}</p>
      </div>
    ...
    <template v-else>
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <p class="text-sm text-accent-500 dark:text-accent-400">{{ t('payment.qr.payInNewWindowHint') }}</p>
          <button v-if="payUrl" class="btn btn-secondary text-sm" @click="reopenPopup">
            {{ t('payment.qr.openPayWindow') }}
          </button>
        </div>
...
const qrBorderClass = computed(() => {
  ...
  return 'border-accent-200 bg-white dark:border-dark-600 dark:bg-dark-800'
})

<template>
  <div
    :class="[
      ...
      enabled ? 'border-accent-200 dark:border-dark-600' : 'border-accent-200 bg-accent-50 opacity-50 dark:border-dark-700 dark:bg-dark-800/50',
    ]"
  ...
  >
    <div :class="[
      ...
      <div class="flex items-center gap-3">
        <div :class="[
          'rounded-md p-1.5',
          provider.enabled && enabled ? 'bg-green-100 dark:bg-green-900/30' : 'bg-accent-100 dark:bg-dark-700',
        ]">
          <Icon
            name="server"
            size="sm"
            :class="provider.enabled && enabled ? 'text-green-600 dark:text-green-400' : 'text-accent-400'"
          />
        </div>
        <span class="text-sm font-medium text-accent-900 dark:text-white">{{ provider.name }}</span>
        <span class="text-xs text-accent-400 dark:text-accent-500">{{ keyLabel }}</span>
        <span v-if="provider.payment_mode" class="text-xs text-accent-400 dark:text-accent-500">· {{ modeLabel }}</span>
        <span v-if="enabled && availableTypes.length" class="text-xs text-accent-300 dark:text-accent-600">|</span>
          ...
          <button
            ...
            :class="[
              ...
              isSelected(pt.value)
                ...
                : 'bg-accent-100 text-accent-400 dark:bg-dark-700 dark:text-accent-500',
            ]"
          >{{ pt.label }}</button>
        </div>
      </div>
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-2 border-l border-accent-200 pl-3 dark:border-dark-600">
          <button type="button" @click="emit('edit')" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-accent-500 transition-colors hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-blue-900/20 dark:hover:text-blue-400">
            <Icon name="edit" size="sm" />
            <span class="text-xs">{{ t('common.edit') }}</span>
          </button>
          <button type="button" @click="emit('delete')" class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-accent-500 transition-colors hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400">

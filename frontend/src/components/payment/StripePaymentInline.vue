    ...
    <template v-else-if="success">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4 py-4">
          ...
          <div class="w-full rounded-xl bg-accent-50 p-4 dark:bg-dark-800">
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                <span class="text-accent-500 dark:text-accent-400">{{ t('payment.orders.orderId') }}</span>
                <span class="font-medium text-accent-900 dark:text-white">#{{ orderId }}</span>
              </div>
              <div v-if="amount > 0" class="flex justify-between">
                <span class="text-accent-500 dark:text-accent-400">{{ t('payment.orders.amount') }}</span>
                <span class="font-medium text-accent-900 dark:text-white">{{ orderType === 'balance' ? '$' : '¥' }}{{ amount.toFixed(2) }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-accent-500 dark:text-accent-400">{{ t('payment.orders.payAmount') }}</span>
                <span class="font-medium text-accent-900 dark:text-white">¥{{ payAmount.toFixed(2) }}</span>
              </div>
            </div>
          </div>

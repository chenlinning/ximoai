    ...
    <form id="provider-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div class="flex flex-wrap items-center gap-x-5 gap-y-2">
        ...
        <div v-if="form.provider_key === 'easypay'" class="flex items-center gap-2">
          <span class="text-xs font-medium text-accent-500 dark:text-accent-400">{{ t('admin.settings.payment.paymentMode') }}</span>
        ...
        <div v-if="availableTypes.length > 1" class="flex items-center gap-2">
          <span class="text-xs font-medium text-accent-500 dark:text-accent-400">{{ t('admin.settings.payment.supportedTypes') }}</span>
      ...
      <div class="border-t border-accent-200 pt-4 dark:border-dark-700">
        <div class="mb-3 flex items-center gap-2">
          ...
          <HelpTooltip v-if="paymentGuide" trigger="click" width-class="w-80">
            <template #trigger>
              <button
                type="button"
                class="inline-flex h-5 w-5 items-center justify-center rounded-full border border-accent-300 text-[11px] font-semibold text-accent-400 transition-colors hover:border-primary-500 hover:text-primary-600 dark:border-dark-500 dark:text-accent-500 dark:hover:border-primary-400 dark:hover:text-primary-400"
            ...
            <div class="space-y-3">
              ...
              >
                ...
                <p><span class="text-accent-300">{{ t('admin.settings.payment.guideOpenLabel') }}</span>{{ item.open }}</p>
                <p><span class="text-accent-300">{{ t('admin.settings.payment.guideCallLabel') }}</span>{{ item.call }}</p>
                <p><span class="text-accent-300">{{ t('admin.settings.payment.guideFallbackLabel') }}</span>{{ item.fallback }}</p>
              </div>
              <p v-if="paymentGuide.note" class="border-t border-white/10 pt-2 text-[11px] text-accent-300">
                {{ paymentGuide.note }}
              </p>
            </div>
          </HelpTooltip>
        </div>
        <p v-if="paymentGuide" class="mb-3 text-xs text-accent-500 dark:text-accent-400">
        ...
        <div class="space-y-3">
          <div v-for="field in resolvedFields" :key="field.key">
            <label class="input-label">
              {{ field.label }}
              <span v-if="field.optional" class="text-xs text-accent-400">({{ t('common.optional') }})</span>
            ...
            <div v-else-if="field.sensitive" class="relative">
              ...
              <button
                ...
                class="absolute inset-y-0 right-0 flex items-center pr-3 text-accent-400 hover:text-accent-600 dark:hover:text-accent-300"
              >
        ...
        <div v-if="callbackPaths" class="mt-4 space-y-3">
          <div v-if="callbackPaths.notifyUrl">
            ...
            <div class="flex">
              ...
              <span class="inline-flex items-center whitespace-nowrap rounded-r-lg border border-accent-300 bg-accent-50 px-3 text-xs text-accent-500 dark:border-dark-600 dark:bg-dark-700 dark:text-accent-400">{{ callbackPaths.notifyUrl }}</span>
            </div>
          </div>
          <div v-if="callbackPaths.returnUrl">
            ...
            <div class="flex">
              ...
              <span class="inline-flex items-center whitespace-nowrap rounded-r-lg border border-accent-300 bg-accent-50 px-3 text-xs text-accent-500 dark:border-dark-600 dark:bg-dark-700 dark:text-accent-400">{{ callbackPaths.returnUrl }}</span>
            </div>
          </div>
        </div>
      ...
      <div v-if="limitableTypes.length" class="border-t border-accent-200 pt-4 dark:border-dark-700">
        <button type="button" @click="limitsExpanded = !limitsExpanded" class="flex w-full items-center justify-between">
          ...
          <svg :class="['h-4 w-4 text-accent-400 transition-transform', limitsExpanded && 'rotate-180']" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
        </button>
        <div v-show="limitsExpanded" class="mt-3 space-y-3">
          <div
            v-for="lt in limitableTypes"
            :key="lt.value"
            class="rounded-lg border border-accent-100 p-3 dark:border-dark-700"
          >
            <p class="mb-2 text-xs font-medium text-accent-700 dark:text-accent-300">{{ lt.label }}</p>
            <div class="grid grid-cols-3 gap-3">
              <div>
                <label class="text-xs text-accent-500 dark:text-accent-400">{{ t('admin.settings.payment.limitSingleMin') }}</label>
                <input
                  type="number"
              ...
              <div>
                <label class="text-xs text-accent-500 dark:text-accent-400">{{ t('admin.settings.payment.limitSingleMax') }}</label>
                <input
                  type="number"
              ...
              <div>
                <label class="text-xs text-accent-500 dark:text-accent-400">{{ t('admin.settings.payment.limitDaily') }}</label>
                <input
                  type="number"
          ...
          <p class="text-xs text-accent-400 dark:text-accent-500">{{ t('admin.settings.payment.limitsHint') }}</p>
        </div>
      </div>
    </form>

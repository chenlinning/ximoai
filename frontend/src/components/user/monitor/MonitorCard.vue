<template>
  <button
    type="button"
    class="group text-left p-5 rounded-2xl min-h-[280px] w-full bg-white/70 backdrop-blur-xl border border-accent-200/80 shadow-card dark:bg-dark-800/60 dark:border-dark-700/70 hover:-translate-y-1 hover:shadow-card-hover dark:hover:border-primary-500/30 hover:border-accent-300 transition-all duration-300 ease-out flex flex-col"
    @click="emit('click')"
  >
    ...
    <div class="flex items-start gap-3">
      ...
      <div class="flex-1 min-w-0">
        <div class="text-base font-semibold truncate text-accent-900 dark:text-accent-100">
          {{ item.name }}
        </div>
        <div class="mt-0.5 flex items-center gap-1.5 min-w-0">
          ...
          <span class="font-mono text-xs truncate text-accent-500 dark:text-accent-400">
            {{ item.primary_model }}
          </span>
          <span
            v-if="item.group_name"
            class="inline-flex items-center rounded-md px-1.5 py-0.5 text-[10px] font-medium bg-accent-100 text-accent-600 dark:bg-dark-700 dark:text-accent-300 flex-shrink-0"
          >
            {{ item.group_name }}
          </span>
        </div>
    ...
    <!-- Divider -->
    <div class="mt-4 border-t border-accent-100 dark:border-dark-700/60"></div>
...
const providerTintClass = computed(() =>
  PROVIDER_TINT[props.item.provider] ?? 'text-accent-500 dark:text-accent-300'
)

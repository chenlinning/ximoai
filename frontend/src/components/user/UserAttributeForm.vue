<template>
      ...
      <div v-else-if="attr.type === 'multi_select'" class="space-y-2">
        ...
        >
          <input
            ...
            class="h-4 w-4 rounded border-accent-300 text-primary-600"
          />
          <span class="text-sm text-accent-700 dark:text-accent-300">{{ opt.label }}</span>
        </label>
      </div>
  ...
  <div v-else-if="loading" class="flex justify-center py-4">
    <svg class="h-5 w-5 animate-spin text-accent-400" fill="none" viewBox="0 0 24 24">

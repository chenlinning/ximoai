          ...
          <div class="min-w-0 flex-1 space-y-5">
            <div class="space-y-3">
              <div class="flex flex-wrap items-center gap-2">
                <h2 class="truncate text-2xl font-semibold text-accent-900 dark:text-white">
                  {{ displayName }}
                </h2>
              ...
              <div class="space-y-1">
                <p class="truncate text-sm text-accent-600 dark:text-accent-300">
                  {{ primaryEmailDisplay }}
                </p>
                <div
                  v-if="sourceHints.length"
                  class="flex flex-wrap gap-2 text-xs text-accent-500 dark:text-accent-400"
                >
                  <span
            ...
            <div class="grid gap-3 sm:grid-cols-3">
              ...
              >
                <p class="text-xs font-medium uppercase tracking-[0.16em] text-accent-400 dark:text-accent-500">
                  {{ t('profile.accountBalance') }}
                </p>
                <p class="mt-1 text-lg font-semibold text-accent-900 dark:text-white">
    ...
    <div class="space-y-6">
      <div data-testid="profile-main-column" class="space-y-6">
        <section
          ...
          class="card border border-accent-100 bg-white/90 p-6 dark:border-dark-700 dark:bg-dark-900/50"
        >
          ...
          <div class="grid gap-6 sm:grid-cols-1 md:grid-cols-2">
            <div class="rounded-3xl border border-accent-100 bg-accent-50/80 p-5 dark:border-dark-700 dark:bg-dark-900/30">
              <ProfileAvatarCard
                :user="user"
                embedded
              />
        ...
        >
          ...
          <div class="mt-5 grid gap-3">
            <div
              v-for="hint in sourceHints"
              :key="hint.key"
              class="flex items-start gap-3 rounded-2xl border border-accent-100 bg-accent-50/80 px-4 py-3 text-sm text-accent-600 dark:border-dark-700 dark:bg-dark-900/30 dark:text-accent-300"
            >
              <Icon name="link" size="sm" class="mt-0.5 text-accent-400 dark:text-accent-500" />
              <span>{{ hint.text }}</span>
            </div>
          </div>
        </section>

    ...
    <div class="flex min-h-full items-center justify-center p-4">
      ...
      <div class="relative w-full max-w-md transform rounded-xl bg-white p-6 shadow-xl transition-all dark:bg-dark-800">
        ...
        <div class="mb-6 text-center">
          <h3 class="text-xl font-semibold text-accent-900 dark:text-white">
        ...
        <div v-if="step === 1" class="space-y-6">
          ...
          <template v-if="setupData">
            <div class="flex justify-center">
              <div class="rounded-lg border border-accent-200 p-4 bg-white dark:border-dark-600 dark:bg-white">
                <img :src="qrCodeDataUrl" alt="QR Code" class="h-48 w-48" />
              </div>
            </div>

            <div class="text-center">
              <p class="text-sm text-accent-500 dark:text-accent-400 mb-2">
                {{ t('profile.totp.manualEntry') }}
              </p>
              <div class="flex items-center justify-center gap-2">
                <code class="rounded bg-accent-100 px-3 py-2 font-mono text-sm dark:bg-dark-700">
                  {{ setupData.secret }}
                </code>
                <button
                  type="button"
                  class="rounded p-1.5 text-accent-500 hover:bg-accent-100 dark:hover:bg-dark-700"
                  @click="copySecret"
                >

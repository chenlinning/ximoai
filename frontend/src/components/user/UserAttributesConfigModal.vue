  ...
  <BaseDialog :show="show" :title="t('admin.users.attributes.title')" width="wide" @close="emit('close')">
    <div class="space-y-4">
      ...
      <div v-else-if="attributes.length === 0" class="py-12 text-center">
        <svg class="mx-auto h-12 w-12 text-accent-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1">
        ...
        <p class="text-xs text-accent-400 dark:text-dark-500">
      ...
      <div v-else class="max-h-96 space-y-2 overflow-y-auto">
        <div
          ...
          :key="attr.id"
          class="flex items-center gap-3 rounded-lg border border-accent-200 bg-white p-3 dark:border-dark-600 dark:bg-dark-800"
        >
          <!-- Drag Handle -->
          <div class="cursor-move text-accent-400 hover:text-accent-600 dark:hover:text-accent-300" :title="t('admin.users.attributes.dragToReorder')">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <span class="font-medium text-accent-900 dark:text-white">{{ attr.name }}</span>
              <span class="rounded bg-accent-100 px-1.5 py-0.5 font-mono text-xs text-accent-500 dark:bg-dark-700 dark:text-dark-400">
                {{ attr.key }}
              </span>
            ...
            <div class="mt-0.5 flex items-center gap-2 text-xs text-accent-500 dark:text-dark-400">
          ...
          <div class="flex items-center gap-1">
            <button
              ...
              class="rounded-lg p-1.5 text-accent-500 hover:bg-accent-100 hover:text-primary-600 dark:hover:bg-dark-700 dark:hover:text-primary-400"
              :title="t('common.edit')"
            >
            ...
            <button
              ...
              class="rounded-lg p-1.5 text-accent-500 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400"
    ...
    <form id="attribute-form" @submit.prevent="handleSave" class="space-y-4">
      ...
      <div v-if="form.type === 'select' || form.type === 'multi_select'" class="space-y-2">
        ...
        <div v-for="(option, index) in form.options" :key="getOptionKey(option)" class="flex items-center gap-2">
          ...
          <button
            ...
            class="rounded-lg p-1.5 text-accent-500 hover:bg-red-50 hover:text-red-600"
          >
      ...
      <div class="flex items-center gap-6">
        <label class="flex items-center gap-2">
          <input v-model="form.required" type="checkbox" class="h-4 w-4 rounded border-accent-300 text-primary-600" />
          <span class="text-sm text-accent-700 dark:text-accent-300">{{ t('admin.users.attributes.required') }}</span>
        </label>
        <label class="flex items-center gap-2">
          <input v-model="form.enabled" type="checkbox" class="h-4 w-4 rounded border-accent-300 text-primary-600" />
          <span class="text-sm text-accent-700 dark:text-accent-300">{{ t('admin.users.attributes.enabled') }}</span>
        </label>
      </div>
    </form>

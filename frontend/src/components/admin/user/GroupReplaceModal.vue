    ...
    <div v-if="oldGroup" class="space-y-4">
      ...
      <div class="rounded-lg border border-accent-200 bg-accent-50 p-3 dark:border-dark-600 dark:bg-dark-800">
        <div class="flex items-center gap-2">
          ...
          <span class="font-medium text-accent-900 dark:text-white">{{ oldGroup.name }}</span>
          <Icon name="arrowRight" size="sm" class="ml-auto text-accent-400" />
      ...
      <div v-if="availableGroups.length > 0" class="max-h-64 space-y-2 overflow-y-auto">
        <label
          ...
          :class="selectedGroupId === group.id
            ...
            : 'border-accent-200 hover:border-accent-300 dark:border-dark-600 dark:hover:border-dark-500'"
        >
          <input
            type="radio"
            :value="group.id"
          ...
          <div class="flex-1">
            ...
            <span class="ml-2 text-xs text-accent-400">{{ group.platform }}</span>
          </div>
        </label>
      </div>

      <!-- 无可选分组 -->
      <div v-else class="py-6 text-center text-sm text-accent-400">

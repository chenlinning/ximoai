<template>
  <BaseDialog :show="show" :title="t('admin.users.balanceHistoryTitle')" width="wide" :close-on-click-outside="true" :z-index="40" @close="$emit('close')">
    <div v-if="user" class="space-y-4">
      ...
      <div class="rounded-xl bg-accent-50 p-4 dark:bg-dark-700">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              <p class="truncate font-medium text-accent-900 dark:text-white">{{ user.email }}</p>
              <span
                v-if="user.username"
          ...
          <div class="flex-shrink-0 text-right">
            <p class="text-xs text-accent-500 dark:text-dark-400">{{ t('admin.users.currentBalance') }}</p>
        ...
        <div class="mt-2.5 flex items-center justify-between border-t border-accent-200/60 pt-2.5 dark:border-dark-600/60">
          <p class="min-w-0 flex-1 truncate text-xs text-accent-500 dark:text-dark-400" :title="user.notes || ''">
            <template v-if="user.notes">{{ t('admin.users.notes') }}: {{ user.notes }}</template>
            <template v-else>&nbsp;</template>
          </p>
          <p class="ml-4 flex-shrink-0 text-xs text-accent-500 dark:text-dark-400">
      ...
      <div class="flex items-center gap-3">
        ...
        <button
          v-if="!hideActions"
          @click="emit('deposit')"
          class="flex items-center gap-2 rounded-lg border border-accent-200 bg-white px-3 py-2 text-sm text-accent-700 transition-colors hover:bg-accent-50 dark:border-dark-600 dark:bg-dark-800 dark:text-accent-300 dark:hover:bg-dark-700"
        >
      ...
      <div v-else-if="history.length === 0" class="py-8 text-center">
        <p class="text-sm text-accent-500">{{ t('admin.users.noBalanceHistory') }}</p>
      </div>
      ...
      <div v-else class="max-h-[28rem] space-y-3 overflow-y-auto">
        <div
          ...
          :key="item.id"
          class="rounded-xl border border-accent-200 bg-white p-4 dark:border-dark-600 dark:bg-dark-800"
        >
          <div class="flex items-start justify-between">
            ...
            <div class="flex items-start gap-3">
              ...
              <div>
                ...
                <p
                  v-if="item.notes"
                  class="mt-0.5 text-xs text-accent-500 dark:text-dark-400"
                  :title="item.notes"
                >
                  {{ item.notes.length > 60 ? item.notes.substring(0, 55) + '...' : item.notes }}
                </p>
                <p class="mt-0.5 text-xs text-accent-400 dark:text-dark-500">
            ...
            <div class="text-right">
              ...
              <p
                v-else
                class="font-mono text-xs text-accent-400 dark:text-dark-500"
              >
                {{ item.code.slice(0, 8) }}...
              </p>
            </div>

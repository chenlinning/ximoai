  ...
  >
      ...
      <div v-else class="max-h-96 overflow-auto rounded-lg border border-accent-200 dark:border-dark-600">
        <table class="min-w-full divide-y divide-accent-200 dark:divide-dark-700">
          ...
          <tbody class="divide-y divide-accent-200 bg-white dark:divide-dark-700 dark:bg-dark-800">
            <tr v-for="profile in profiles" :key="profile.id" class="hover:bg-accent-50 dark:hover:bg-dark-700">
              <td class="px-3 py-2">
                <div class="font-medium text-accent-900 dark:text-white text-sm">{{ profile.name }}</div>
              </td>
              <td class="px-3 py-2">
                <div v-if="profile.description" class="text-sm text-accent-500 dark:text-accent-400 max-w-xs truncate">
                  {{ profile.description }}
                </div>
                <div v-else class="text-xs text-accent-400 dark:text-accent-600">—</div>
              </td>
              <td class="px-3 py-2">
                <Icon
                  :name="profile.enable_grease ? 'check' : 'lock'"
                  size="sm"
                  :class="profile.enable_grease ? 'text-green-500' : 'text-accent-400'"
                />
              </td>
              <td class="px-3 py-2">
                <div v-if="profile.alpn_protocols?.length" class="flex flex-wrap gap-1">
                  ...
                  <span v-if="profile.alpn_protocols.length > 3" class="text-xs text-accent-500">
    ...
    >
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <hr class="border-accent-200 dark:border-dark-600" />
        ...
        <div class="flex items-center gap-3">
          <button
            ...
            :class="[
              ...
              form.enable_grease ? 'bg-primary-600' : 'bg-accent-200 dark:bg-dark-600'
            ]"
          >
            <span
              :class="[

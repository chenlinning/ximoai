<template>
  <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
    <div class="card p-4 flex items-center gap-3">
      ...
      <div>
        <p class="text-xs font-medium text-accent-500">{{ t('usage.totalRequests') }}</p>
        <p class="text-xl font-bold">{{ stats?.total_requests?.toLocaleString() || '0' }}</p>
        <p class="text-xs text-accent-400">{{ t('usage.inSelectedRange') }}</p>
      </div>
    </div>
    <div class="card p-4 flex items-center gap-3">
      ...
      <div>
        <p class="text-xs font-medium text-accent-500">{{ t('usage.totalTokens') }}</p>
    ...
    <div class="card p-4 flex items-center gap-3">
      ...
      <div class="min-w-0 flex-1">
        <p class="text-xs font-medium text-accent-500">{{ t('usage.totalCost') }}</p>
    ...
    <div class="card p-4 flex items-center gap-3">
      ...
      <div><p class="text-xs font-medium text-accent-500">{{ t('usage.avgDuration') }}</p><p class="text-xl font-bold">{{ formatDuration(stats?.average_duration_ms || 0) }}</p></div>
    </div>
  </div>
</template>

<template>
  <BaseDialog :show="show" :title="t('admin.usage.cleanup.title')" width="wide" @close="handleClose">
    <div class="space-y-4">
      ...
      <div class="rounded-xl border border-accent-200 p-4 dark:border-dark-700">
        <div class="flex items-center justify-between">
          <h4 class="text-sm font-semibold text-accent-700 dark:text-accent-200">
        ...
        <div class="mt-3 space-y-2">
          <div v-if="tasksLoading" class="text-sm text-accent-500 dark:text-accent-400">
            {{ t('admin.usage.cleanup.loadingTasks') }}
          </div>
          <div v-else-if="tasks.length === 0" class="text-sm text-accent-500 dark:text-accent-400">
          ...
          <div v-else class="space-y-2">
            <div
              v-for="task in tasks"
              :key="task.id"
              class="flex flex-col gap-2 rounded-lg border border-accent-100 px-3 py-2 text-sm text-accent-600 dark:border-dark-700 dark:text-accent-300"
            >
              <div class="flex flex-wrap items-center justify-between gap-2">
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-xs text-accent-400">#{{ task.id }}</span>
                  <button
                    v-if="canCancel(task)"
                    type="button"
              ...
              <div class="flex flex-wrap items-center gap-4 text-xs text-accent-500 dark:text-accent-400">
...
const statusClass = (status: string) => {
  const map: Record<string, string> = {
    ...
    canceled: 'bg-accent-200 text-accent-600 dark:bg-dark-600 dark:text-accent-300'
  }
  return map[status] || 'bg-accent-100 text-accent-600'
}

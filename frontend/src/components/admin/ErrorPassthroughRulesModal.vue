  ...
  >
    <div class="space-y-4">
      ...
      <div v-else-if="rules.length === 0" class="py-8 text-center">
        <div class="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-accent-100 dark:bg-dark-700">
          <Icon name="shield" size="lg" class="text-accent-400" />
        </div>
        <h4 class="mb-1 text-sm font-medium text-accent-900 dark:text-white">
      ...
      <div v-else class="max-h-96 overflow-auto rounded-lg border border-accent-200 dark:border-dark-600">
        <table class="min-w-full divide-y divide-accent-200 dark:divide-dark-700">
          <thead class="sticky top-0 bg-accent-50 dark:bg-dark-700">
            <tr>
              <th class="px-3 py-2 text-left text-xs font-medium uppercase text-accent-500 dark:text-accent-400">
          ...
          <tbody class="divide-y divide-accent-200 bg-white dark:divide-dark-700 dark:bg-dark-800">
            <tr v-for="rule in rules" :key="rule.id" class="hover:bg-accent-50 dark:hover:bg-dark-700">
              <td class="whitespace-nowrap px-3 py-2">
                <span class="inline-flex h-5 w-5 items-center justify-center rounded bg-accent-100 text-xs font-medium text-accent-700 dark:bg-dark-600 dark:text-accent-300">
                  {{ rule.priority }}
                </span>
              </td>
              <td class="px-3 py-2">
                <div class="font-medium text-accent-900 dark:text-white text-sm">{{ rule.name }}</div>
                <div v-if="rule.description" class="mt-0.5 text-xs text-accent-500 dark:text-accent-400 max-w-xs truncate">
                  {{ rule.description }}
                </div>
              </td>
              <td class="px-3 py-2">
                ...
                <div class="mt-0.5 text-xs text-accent-500 dark:text-accent-400">
                  {{ t('admin.errorPassthrough.matchMode.' + rule.match_mode) }}
                </div>
              </td>
              <td class="px-3 py-2">
                <div v-if="rule.platforms.length === 0" class="text-xs text-accent-500 dark:text-accent-400">
                ...
                <div v-else class="flex flex-wrap gap-1">
                  ...
                  <span v-if="rule.platforms.length > 2" class="text-xs text-accent-500">
                ...
                <div class="text-xs space-y-0.5">
                  <div class="flex items-center gap-1">
                    <Icon
                      ...
                      size="xs"
                      :class="rule.passthrough_code ? 'text-green-500' : 'text-accent-400'"
                    />
                  ...
                  <div class="flex items-center gap-1">
                    <Icon
                      ...
                      size="xs"
                      :class="rule.passthrough_body ? 'text-green-500' : 'text-accent-400'"
                    />
                    <span class="text-accent-600 dark:text-accent-400">
              ...
              <td class="px-3 py-2">
                <button
                  ...
                  :class="[
                    ...
                    rule.enabled ? 'bg-primary-600' : 'bg-accent-200 dark:bg-dark-600'
                  ]"
                >
                  <span
                    :class="[
              ...
              <td class="px-3 py-2">
                <div class="flex items-center gap-1">
                  <button
                    ...
                    class="p-1 text-accent-500 hover:text-primary-600 dark:hover:text-primary-400"
                    :title="t('common.edit')"
                  >
                  ...
                  <button
                    ...
                    class="p-1 text-accent-500 hover:text-red-600 dark:hover:text-red-400"
    ...
    >
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div class="rounded-lg border border-accent-200 p-3 dark:border-dark-600">
          <h4 class="mb-2 text-sm font-medium text-accent-900 dark:text-white">
            ...
            <div class="mt-1 space-y-2">
              ...
              >
                <input
                  ...
                  class="mt-0.5 h-3.5 w-3.5 border-accent-300 text-primary-600 focus:ring-primary-500"
                />
                <div class="flex-1">
                  <span class="text-xs font-medium text-accent-700 dark:text-accent-300">{{ option.label }}</span>
                  <p class="text-xs text-accent-500 dark:text-accent-400">{{ option.description }}</p>
                </div>
              </label>
            </div>
          </div>
          ...
          <div class="mt-3">
            ...
            <div class="flex flex-wrap gap-3">
              ...
              >
                ...
                />
                <span class="text-xs text-accent-700 dark:text-accent-300">{{ platform.label }}</span>
              </label>
            </div>
        ...
        <div class="flex items-center gap-1.5">
          <input
            ...
            class="h-3.5 w-3.5 rounded border-accent-300 text-yellow-600 focus:ring-yellow-500"
          />
          <span class="text-xs font-medium text-accent-700 dark:text-accent-300">

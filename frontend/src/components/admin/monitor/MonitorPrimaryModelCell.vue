<template>
  <div class="flex items-center gap-2">
    <span class="text-sm text-accent-900 dark:text-accent-100">{{ row.primary_model }}</span>
    <HelpTooltip>
      <template #trigger>
        <span
      ...
      <div class="space-y-2">
        <div class="text-xs font-semibold text-accent-100">
          {{ row.primary_model }}
          <span
        ...
        <div v-if="(row.extra_models?.length ?? 0) === 0" class="text-[11px] text-accent-300">
          {{ t('monitorCommon.extraModelsEmpty') }}
        </div>
        <div v-else class="space-y-1">
          <div class="text-[11px] font-semibold uppercase tracking-wide text-accent-400">
            {{ t('monitorCommon.extraModelsHeader') }}
          </div>
          <table class="w-full text-left text-[11px]">
            <thead>
              <tr class="text-accent-400">
            ...
            <tbody>
              <tr v-for="m in (row.extra_models_status || [])" :key="m.model">
                <td class="py-0.5 pr-2 text-accent-100">{{ m.model }}</td>
                ...
                <td class="py-0.5 text-accent-100">{{ formatLatency(m.latency_ms) }}</td>
              </tr>
            </tbody>
          </table>
        </div>

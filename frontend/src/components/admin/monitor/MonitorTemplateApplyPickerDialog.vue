<template>
  <BaseDialog
    ...
    @close="$emit('close')"
  >
    <p class="mb-3 text-sm text-accent-600 dark:text-accent-400">
      {{ t('admin.channelMonitor.template.applyPickerHint') }}
    </p>

    <div v-if="loading" class="py-6 text-center text-sm text-accent-400">
      {{ t('common.loading') }}
    </div>

    <div v-else-if="monitors.length === 0" class="py-6 text-center text-sm text-accent-400">
    ...
    <div v-else>
      ...
      <div class="mb-2 flex items-center gap-3 text-xs">
        ...
        <button
          type="button"
          class="text-accent-500 hover:underline dark:text-accent-400"
          @click="selectNone"
        >
          {{ t('admin.channelMonitor.template.selectNone') }}
        </button>
        <span class="ml-auto text-accent-500 dark:text-accent-400">
      ...
      <ul class="max-h-80 divide-y divide-accent-100 overflow-y-auto rounded-lg border border-accent-200 dark:divide-dark-700 dark:border-dark-700">
        <li
          v-for="m in monitors"
          :key="m.id"
          class="flex cursor-pointer items-center gap-3 px-3 py-2 hover:bg-accent-50 dark:hover:bg-dark-800"
          @click="toggle(m.id)"
        >
          <input
            type="checkbox"
          ...
          />
          <span class="font-medium text-accent-900 dark:text-white">{{ m.name }}</span>
          <span class="text-xs text-accent-400">{{ m.provider }}</span>
          <span
            v-if="!m.enabled"
            class="ml-auto rounded bg-accent-100 px-1.5 py-0.5 text-xs text-accent-500 dark:bg-dark-700 dark:text-accent-400"
          >

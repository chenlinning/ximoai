<template>
  <div class="flex items-start gap-2 rounded border p-2"
       :class="isEmpty ? 'border-red-400 bg-red-50 dark:border-red-500 dark:bg-red-950/20' : 'border-accent-200 bg-white dark:border-dark-500 dark:bg-dark-700'">
    <!-- Token mode: context range + prices ($/MTok) -->
    <template v-if="mode === 'token'">
      <div class="w-20">
        <label class="text-xs text-accent-400">Min</label>
      ...
      <div class="w-20">
        <label class="text-xs text-accent-400">Max <span class="text-accent-300">(含)</span></label>
      ...
      <div class="flex-1">
        <label class="text-xs text-accent-400">{{ t('admin.channels.form.inputPrice', '输入') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-accent-300">$/M</span></label>
      ...
      <div class="flex-1">
        <label class="text-xs text-accent-400">{{ t('admin.channels.form.outputPrice', '输出') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-accent-300">$/M</span></label>
        <input :value="interval.output_price" @input="emitField('output_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
      <div class="flex-1">
        <label class="text-xs text-accent-400">{{ t('admin.channels.form.cacheWritePrice', '缓存W') }} <span class="text-accent-300">$/M</span></label>
        <input :value="interval.cache_write_price" @input="emitField('cache_write_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
      <div class="flex-1">
        <label class="text-xs text-accent-400">{{ t('admin.channels.form.cacheReadPrice', '缓存R') }} <span class="text-accent-300">$/M</span></label>
    ...
    <template v-else>
      <div class="w-24">
        <label class="text-xs text-accent-400">
      ...
      <div class="flex-1">
        <label class="text-xs text-accent-400">{{ t('admin.channels.form.perRequestPrice', '单次价格') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-accent-300">$</span></label>
        <input :value="interval.per_request_price" @input="emitField('per_request_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
    </template>

    <button type="button" @click="emit('remove')" class="mt-4 rounded p-0.5 text-accent-400 hover:text-red-500">
      <Icon name="x" size="sm" />
    </button>
  </div>
</template>

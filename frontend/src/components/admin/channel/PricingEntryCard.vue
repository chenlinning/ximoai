<template>
  <div class="rounded-lg border border-accent-200 bg-accent-50 p-3 dark:border-dark-600 dark:bg-dark-800">
    ...
    >
      <Icon
        ...
        size="sm"
        :stroke-width="2"
        class="flex-shrink-0 text-accent-400 transition-transform duration-200"
      />
      ...
      <div v-if="collapsed" class="flex min-w-0 flex-1 items-center gap-2 overflow-hidden">
        ...
        <div class="flex min-w-0 flex-1 flex-wrap items-center gap-1">
          ...
          <span
            ...
            class="whitespace-nowrap text-xs text-accent-400"
          >
            +{{ entry.models.length - 3 }}
          </span>
          <span
            v-if="entry.models.length === 0"
            class="text-xs italic text-accent-400"
          >
      ...
      <div v-else class="flex-1 text-xs font-medium text-accent-500 dark:text-accent-400">
      ...
      <button
        ...
        class="flex-shrink-0 rounded p-1 text-accent-400 hover:text-red-500"
      >
    ...
    >
      <div class="collapsible-inner">
        ...
        <div class="mt-3 flex items-start gap-2">
          <div class="flex-1">
            <label class="text-xs font-medium text-accent-500 dark:text-accent-400">
        ...
        <div v-if="entry.billing_mode === 'token'">
          ...
          <label class="mt-3 block text-xs font-medium text-accent-500 dark:text-accent-400">
            {{ t('admin.channels.form.defaultPrices', '默认价格（未命中区间时使用）') }}
            <span class="ml-1 font-normal text-accent-400">$/MTok</span>
          </label>
          <div class="mt-1 grid grid-cols-2 gap-2 sm:grid-cols-5">
            <div>
              <label class="text-xs text-accent-400">{{ t('admin.channels.form.inputPrice', '输入') }}</label>
            ...
            <div>
              <label class="text-xs text-accent-400">{{ t('admin.channels.form.outputPrice', '输出') }}</label>
              <input :value="entry.output_price" @input="emitField('output_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.pricePlaceholder', '默认')" />
            </div>
            <div>
              <label class="text-xs text-accent-400">{{ t('admin.channels.form.cacheWritePrice', '缓存写入') }}</label>
              <input :value="entry.cache_write_price" @input="emitField('cache_write_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.pricePlaceholder', '默认')" />
            </div>
            <div>
              <label class="text-xs text-accent-400">{{ t('admin.channels.form.cacheReadPrice', '缓存读取') }}</label>
              <input :value="entry.cache_read_price" @input="emitField('cache_read_price', ($event.target as HTMLInputElement).value)"
                type="number" step="any" min="0" class="input mt-0.5 text-sm" :placeholder="t('admin.channels.form.pricePlaceholder', '默认')" />
            </div>
            <div>
              <label class="text-xs text-accent-400">{{ t('admin.channels.form.imageTokenPrice', '图片输出') }}</label>
          ...
          <div class="mt-3">
            <div class="flex items-center justify-between">
              <label class="text-xs font-medium text-accent-500 dark:text-accent-400">
                ...
                <span class="ml-1 font-normal text-accent-400">(min, max]</span>
              </label>
        ...
        <div v-else-if="entry.billing_mode === 'per_request'">
          ...
          <label class="mt-3 block text-xs font-medium text-accent-500 dark:text-accent-400">
            ...
            <span class="ml-1 font-normal text-accent-400">$</span>
          </label>
          <div class="mt-1 w-48">
          ...
          <div v-else class="mt-2 rounded border border-dashed border-accent-300 p-3 text-center text-xs text-accent-400 dark:border-dark-500">

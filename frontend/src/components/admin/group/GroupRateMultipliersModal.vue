<template>
  <BaseDialog :show="show" :title="t('admin.groups.rateMultipliersTitle')" width="wide" @close="handleClose">
    <div v-if="group" class="space-y-4">
      <!-- 分组信息 -->
      <div class="flex flex-wrap items-center gap-3 rounded-lg bg-accent-50 px-4 py-2.5 text-sm dark:bg-dark-700">
        <span class="inline-flex items-center gap-1.5" :class="platformColorClass">
          <PlatformIcon :platform="group.platform" size="sm" />
          {{ t('admin.groups.platforms.' + group.platform) }}
        </span>
        <span class="text-accent-400">|</span>
        <span class="font-medium text-accent-900 dark:text-white">{{ group.name }}</span>
        <span class="text-accent-400">|</span>
      ...
      <!-- 操作区 -->
      <div class="rounded-lg border border-accent-200 p-3 dark:border-dark-600">
        <!-- 添加用户 -->
        <h4 class="mb-2 text-sm font-medium text-accent-700 dark:text-accent-300">
        ...
        <div class="flex items-end gap-2">
          <div class="relative flex-1">
            ...
            <div
              ...
              class="absolute left-0 right-0 top-full z-10 mt-1 max-h-48 overflow-y-auto rounded-lg border border-accent-200 bg-white shadow-lg dark:border-dark-500 dark:bg-dark-700"
            >
              <button
                v-for="user in searchResults"
                :key="user.id"
                type="button"
                class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-sm hover:bg-accent-50 dark:hover:bg-dark-600"
                @click="selectUser(user)"
              >
                <span class="text-accent-400">#{{ user.id }}</span>
                <span class="text-accent-900 dark:text-white">{{ user.username || user.email }}</span>
                <span v-if="user.username" class="text-xs text-accent-400">{{ user.email }}</span>
              </button>
            </div>
          </div>
          <div class="w-24">
        ...
        <!-- 批量调整 + 全部清空 -->
        <div v-if="localEntries.length > 0" class="mt-3 flex items-center gap-3 border-t border-accent-100 pt-3 dark:border-dark-600">
          <span class="text-xs font-medium text-accent-500 dark:text-accent-400">{{ t('admin.groups.batchAdjust') }}</span>
          <div class="flex items-center gap-1.5">
            ...
            <input
              ...
              min="0"
              autocomplete="off"
              class="hide-spinner w-20 rounded border border-accent-200 bg-white px-2 py-1 text-center text-sm transition-colors focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500/20 dark:border-dark-500 dark:bg-dark-700 dark:focus:border-primary-500"
              placeholder="0.5"
            />
            <button
              type="button"
      ...
      <div v-else>
        ...
        <div v-if="localEntries.length === 0" class="py-6 text-center text-sm text-accent-400 dark:text-accent-500">
          {{ t('admin.groups.noRateMultipliers') }}
        </div>

        <div v-else>
          <!-- 表格 -->
          <div class="overflow-hidden rounded-lg border border-accent-200 dark:border-dark-600">
            <div class="max-h-[420px] overflow-y-auto">
              <table class="w-full text-sm">
                <thead class="sticky top-0 z-[1]">
                  <tr class="border-b border-accent-200 bg-accent-50 dark:border-dark-600 dark:bg-dark-700">
                    <th class="px-3 py-2 text-left text-xs font-medium text-accent-500 dark:text-accent-400">{{ t('admin.groups.columns.userEmail') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-accent-500 dark:text-accent-400">ID</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-accent-500 dark:text-accent-400">{{ t('admin.groups.columns.userName') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-accent-500 dark:text-accent-400">{{ t('admin.groups.columns.userNotes') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-accent-500 dark:text-accent-400">{{ t('admin.groups.columns.userStatus') }}</th>
                    <th class="px-3 py-2 text-left text-xs font-medium text-accent-500 dark:text-accent-400">{{ t('admin.groups.columns.rateMultiplier') }}</th>
                ...
                <tbody class="divide-y divide-accent-100 dark:divide-dark-600">
                  <tr
                    v-for="entry in paginatedLocalEntries"
                    :key="entry.user_id"
                    class="hover:bg-accent-50 dark:hover:bg-dark-700/50"
                  >
                    <td class="px-3 py-2 text-accent-600 dark:text-accent-400">{{ entry.user_email }}</td>
                    <td class="whitespace-nowrap px-3 py-2 text-accent-400 dark:text-accent-500">{{ entry.user_id }}</td>
                    <td class="whitespace-nowrap px-3 py-2 text-accent-900 dark:text-white">{{ entry.user_name || '-' }}</td>
                    <td class="max-w-[160px] truncate px-3 py-2 text-accent-500 dark:text-accent-400" :title="entry.user_notes">{{ entry.user_notes || '-' }}</td>
                      ...
                      <span
                        :class="[
                          ...
                          entry.user_status === 'active'
                            ...
                            : 'bg-accent-100 text-accent-600 dark:bg-dark-600 dark:text-accent-400'
                        ]"
                      >
                        {{ entry.user_status }}
                      </span>
                    ...
                    <td class="whitespace-nowrap px-3 py-2">
                      <input
                        ...
                        class="hide-spinner w-20 rounded border border-accent-200 bg-white px-2 py-1 text-center text-sm font-medium transition-colors focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500/20 dark:border-dark-500 dark:bg-dark-700 dark:focus:border-primary-500"
                    ...
                    <td class="px-2 py-2">
                      <button
                        type="button"
                        class="rounded p-1 text-accent-400 transition-colors hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 dark:hover:text-red-400"
      ...
      <!-- 底部操作栏 -->
      <div class="flex items-center gap-3 border-t border-accent-200 pt-4 dark:border-dark-600">
        <!-- 左侧：未保存提示 + 撤销 -->

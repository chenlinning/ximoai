<template>
  ...
  >
    <div class="space-y-4">
      ...
      >
        <div class="mb-3 text-sm font-medium text-accent-700 dark:text-accent-300">
          {{ t('admin.scheduledTests.addPlan') }}
        </div>
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            <label class="mb-1 block text-xs font-medium text-accent-600 dark:text-accent-400">
          ...
          <div>
            <label class="mb-1 flex items-center gap-1 text-xs font-medium text-accent-600 dark:text-accent-400">
              {{ t('admin.scheduledTests.cronExpression') }}
              <HelpTooltip>
                <template #trigger>
                  <span class="inline-flex h-4 w-4 cursor-help items-center justify-center rounded-full border border-accent-400/70 text-[10px] font-semibold text-accent-400 transition-colors hover:border-primary-500 hover:text-primary-600 dark:border-accent-500 dark:text-accent-500 dark:hover:border-primary-400 dark:hover:text-primary-400">
                    ?
                  </span>
                </template>
          ...
          <div class="flex items-end">
            <div>
              ...
              <p class="mt-0.5 text-xs text-accent-400 dark:text-accent-500">
        ...
        <div class="mt-3 flex justify-end gap-2">
          <button
            ...
            class="rounded-lg bg-accent-100 px-3 py-1.5 text-sm font-medium text-accent-700 transition-colors hover:bg-accent-200 dark:bg-dark-600 dark:text-accent-300 dark:hover:bg-dark-500"
          >
            {{ t('common.cancel') }}
          </button>
          <button
      ...
      <div v-if="loading" class="flex items-center justify-center py-8">
        <Icon name="refresh" size="md" class="animate-spin text-accent-400" :stroke-width="2" />
        <span class="ml-2 text-sm text-accent-500">{{ t('common.loading') }}...</span>
      </div>

      <!-- Empty State -->
      <div
        v-else-if="plans.length === 0"
        class="rounded-xl border border-dashed border-accent-300 py-10 text-center dark:border-dark-600"
      >
        <Icon name="calendar" size="lg" class="mx-auto mb-2 text-accent-400" :stroke-width="1.5" />
        <p class="text-sm text-accent-500 dark:text-accent-400">
      ...
      <div v-else class="space-y-3">
        <div
          v-for="plan in plans"
          :key="plan.id"
          class="rounded-xl border border-accent-200 bg-white transition-all dark:border-dark-600 dark:bg-dark-800"
        >
          <!-- Plan Header -->
          <div
          ...
          >
            <div class="flex flex-1 items-center gap-4">
              <!-- Model -->
              <div class="min-w-0">
                <div class="text-sm font-medium text-accent-900 dark:text-accent-100">
                  {{ plan.model_id }}
                </div>
                <div class="mt-0.5 font-mono text-xs text-accent-500 dark:text-accent-400">
            ...
            <div class="flex items-center gap-3">
              <!-- Last Run -->
              <div v-if="plan.last_run_at" class="hidden text-right text-xs text-accent-500 dark:text-accent-400 sm:block">
                <div>{{ t('admin.scheduledTests.lastRun') }}</div>
                <div>{{ formatDateTime(plan.last_run_at) }}</div>
              </div>

              <!-- Next Run -->
              <div v-if="plan.next_run_at" class="hidden text-right text-xs text-accent-500 dark:text-accent-400 sm:block">
              ...
              <div class="flex items-center gap-1" @click.stop>
                <button
                  ...
                  class="rounded-lg p-1.5 text-accent-400 transition-colors hover:bg-blue-50 hover:text-blue-500 dark:hover:bg-blue-900/20"
                ...
                <button
                  ...
                  class="rounded-lg p-1.5 text-accent-400 transition-colors hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-900/20"
              ...
              <Icon
                ...
                size="sm"
                :class="[
                  'text-accent-400 transition-transform duration-200',
          ...
          <div
            ...
            @click.stop
          >
            <div class="mb-2 text-xs font-medium text-accent-600 dark:text-accent-400">
          ...
          <div
            ...
            class="border-t border-accent-100 px-4 py-3 dark:border-dark-700"
          >
            <div class="mb-2 text-xs font-medium text-accent-600 dark:text-accent-400">
            ...
            <div v-if="loadingResults" class="flex items-center justify-center py-4">
              <Icon name="refresh" size="sm" class="animate-spin text-accent-400" :stroke-width="2" />
              <span class="ml-2 text-xs text-accent-500">{{ t('common.loading') }}...</span>
            </div>

            <!-- No Results -->
            <div
              v-else-if="results.length === 0"
              class="py-4 text-center text-xs text-accent-500 dark:text-accent-400"
            >
            ...
            <div v-else class="max-h-64 space-y-2 overflow-y-auto">
              <div
                ...
                :key="result.id"
                class="rounded-lg border border-accent-100 bg-accent-50 p-3 dark:border-dark-700 dark:bg-dark-900"
              >
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    ...
                    <!-- Latency -->
                    <span v-if="result.latency_ms > 0" class="text-xs text-accent-500 dark:text-accent-400">
                ...
                <div v-else-if="result.response_text" class="mt-2">
                  <div
                    class="cursor-pointer text-xs font-medium text-accent-600 dark:text-accent-400"
                    @click="toggleResultDetail(result.id)"
                  >
                  ...
                  <pre
                    v-if="expandedResultIds.has(result.id)"
                    class="mt-1 max-h-32 overflow-auto whitespace-pre-wrap rounded bg-accent-100 p-2 text-xs text-accent-700 dark:bg-dark-800 dark:text-accent-300"

  ...
  <div class="space-y-4">
    ...
    <DataTable :columns="columns" :data="orders" :loading="loading">
      ...
      <template #cell-pay_amount="{ value, row }">
        <div class="text-sm">
          <span class="font-medium text-accent-900 dark:text-white">¥{{ value.toFixed(2) }}</span>
          <span v-if="row.fee_rate > 0" class="ml-1 text-xs text-accent-400" :title="t('payment.orders.fee') + ': ' + row.fee_rate + '%'">
            ({{ row.fee_rate }}%)
          </span>
          <div v-if="row.amount !== row.pay_amount" class="text-xs text-accent-500">
      ...
      <template #cell-created_at="{ value }">
        <span class="text-xs text-accent-500 dark:text-accent-400">{{ formatDateTime(value) }}</span>
      </template>
      ...
      <template #cell-actions="{ row }">
        <div class="flex items-center gap-2">
          <button
            ...
            class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-accent-500 transition-colors hover:bg-accent-50 hover:text-accent-700 dark:hover:bg-accent-800/50 dark:hover:text-accent-300"
          >
          ...
          <button
            ...
            class="flex flex-col items-center gap-0.5 rounded-lg p-1.5 text-accent-500 transition-colors hover:bg-yellow-50 hover:text-yellow-600 dark:hover:bg-yellow-900/20 dark:hover:text-yellow-400"
          >
            <Icon name="x" size="sm" />

<template>
  <div>
    ...
    <button
      @click="openModal"
      class="relative flex h-9 w-9 items-center justify-center rounded-lg text-accent-600 transition-all hover:bg-accent-100 hover:scale-105 dark:text-accent-400 dark:hover:bg-dark-800"
      ...
      <Transition name="modal-fade">
        ...
        >
          ...
          >
            ...
            <div class="relative overflow-hidden border-b border-accent-100/80 bg-gradient-to-br from-blue-50/50 to-indigo-50/30 px-6 py-5 dark:border-dark-700/50 dark:from-blue-900/10 dark:to-indigo-900/5">
              <div class="relative z-10 flex items-start justify-between">
                <div>
                  ...
                  <p v-if="unreadCount > 0" class="mt-2 text-sm text-accent-600 dark:text-accent-400">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <button
                    @click="closeModal"
                    class="flex h-9 w-9 items-center justify-center rounded-lg bg-white/50 text-accent-500 backdrop-blur-sm transition-all hover:bg-white hover:text-accent-700 dark:bg-dark-700/50 dark:text-accent-400 dark:hover:bg-dark-700 dark:hover:text-accent-300"
            ...
            <div class="max-h-[65vh] overflow-y-auto">
              ...
              <div v-if="loading" class="flex items-center justify-center py-16">
                <div class="relative">
                  <div class="h-12 w-12 animate-spin rounded-full border-4 border-accent-200 border-t-blue-600 dark:border-dark-600 dark:border-t-blue-400"></div>
              ...
              <div v-else-if="announcements.length > 0">
                <div
                  ...
                  :key="item.id"
                  class="group relative flex items-center gap-4 border-b border-accent-100 px-6 py-4 transition-all hover:bg-accent-50 dark:border-dark-700 dark:hover:bg-dark-700/30"
                ...
                >
                  ...
                  <div class="flex h-10 w-10 flex-shrink-0 items-center justify-center">
                    ...
                    <div
                      v-else
                      class="flex h-10 w-10 items-center justify-center rounded-xl bg-accent-100 text-accent-400 dark:bg-dark-700 dark:text-accent-600"
                    >
                  ...
                  <div class="flex min-w-0 flex-1 items-center justify-between gap-4">
                    <div class="min-w-0 flex-1">
                      <h3 class="truncate text-sm font-medium text-accent-900 dark:text-white">
                        {{ item.title }}
                      </h3>
                      <div class="mt-1 flex items-center gap-2">
                        <time class="text-xs text-accent-500 dark:text-accent-400">
                    ...
                    <div class="flex-shrink-0">
                      <svg
                        class="h-5 w-5 text-accent-400 transition-transform group-hover:translate-x-1 dark:text-accent-600"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        stroke-width="2"
              ...
              <div v-else class="flex flex-col items-center justify-center py-16">
                <div class="relative mb-4">
                  <div class="flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-accent-100 to-accent-200 dark:from-dark-700 dark:to-dark-600">
                    <Icon name="inbox" size="xl" class="text-accent-400 dark:text-accent-500" />
                  </div>
                ...
                <p class="text-sm font-medium text-accent-900 dark:text-white">{{ t('announcements.empty') }}</p>
                <p class="mt-1 text-xs text-accent-500 dark:text-accent-400">{{ t('announcements.emptyDescription') }}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <Transition name="modal-fade">
        ...
        >
          ...
          >
            ...
            <div class="relative overflow-hidden border-b border-accent-100 bg-gradient-to-br from-blue-50/80 via-indigo-50/50 to-purple-50/30 px-8 py-6 dark:border-dark-700 dark:from-blue-900/20 dark:via-indigo-900/10 dark:to-purple-900/5">
              ...
              <div class="relative z-10 flex items-start justify-between gap-4">
                <div class="flex-1 min-w-0">
                  ...
                  <!-- Title -->
                  <h2 class="mb-3 text-2xl font-bold leading-tight text-accent-900 dark:text-white">
                    {{ selectedAnnouncement.title }}
                  </h2>

                  <!-- Meta Info -->
                  <div class="flex items-center gap-4 text-sm text-accent-600 dark:text-accent-400">
                ...
                <button
                  @click="closeDetail"
                  class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-white/50 text-accent-500 backdrop-blur-sm transition-all hover:bg-white hover:text-accent-700 hover:shadow-lg dark:bg-dark-700/50 dark:text-accent-400 dark:hover:bg-dark-700 dark:hover:text-accent-300"
                  :aria-label="t('common.close')"
                >
                  <Icon name="x" size="md" />
                </button>
            ...
            <div class="border-t border-accent-100 bg-accent-50/50 px-8 py-5 dark:border-dark-700 dark:bg-dark-900/30">
              <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                  <button
                    @click="closeDetail"
                    class="rounded-xl border border-accent-300 bg-white px-5 py-2.5 text-sm font-medium text-accent-700 shadow-sm transition-all hover:bg-accent-50 hover:shadow dark:border-dark-600 dark:bg-dark-700 dark:text-accent-300 dark:hover:bg-dark-600"
                  >
                    {{ t('common.close') }}
                  </button>
                  <button
...
.markdown-body h1 {
  @apply mb-6 mt-8 border-b border-accent-200 pb-3 text-3xl font-bold text-accent-900 dark:border-dark-600 dark:text-white;
}

.markdown-body h2 {
  @apply mb-4 mt-7 border-b border-accent-100 pb-2 text-2xl font-bold text-accent-900 dark:border-dark-700 dark:text-white;
}

.markdown-body h3 {
  @apply mb-3 mt-6 text-xl font-semibold text-accent-900 dark:text-white;
}

.markdown-body h4 {
  @apply mb-2 mt-5 text-lg font-semibold text-accent-900 dark:text-white;
}
...
.markdown-body blockquote {
  @apply relative my-5 border-l-4 border-blue-500 bg-blue-50/50 py-3 pl-5 pr-4 italic text-accent-700 dark:border-blue-400 dark:bg-blue-900/10 dark:text-accent-300;
}
...
.markdown-body code {
  @apply rounded-lg bg-accent-100 px-2 py-1 text-[13px] font-mono text-pink-600 dark:bg-dark-700 dark:text-pink-400;
}

.markdown-body pre {
  @apply my-5 overflow-x-auto rounded-xl border border-accent-200 bg-accent-50 p-5 dark:border-dark-600 dark:bg-dark-900/50;
}

.markdown-body pre code {
  @apply bg-transparent p-0 text-[13px] text-accent-800 dark:text-accent-200;
}

.markdown-body hr {
  @apply my-8 border-0 border-t-2 border-accent-200 dark:border-dark-700;
}

.markdown-body table {
  @apply mb-5 w-full overflow-hidden rounded-lg border border-accent-200 dark:border-dark-600;
}

.markdown-body th,
.markdown-body td {
  @apply border-r border-b border-accent-200 px-4 py-3 text-left dark:border-dark-600;
}
...
.markdown-body th {
  @apply bg-gradient-to-br from-blue-50 to-indigo-50 font-semibold text-accent-900 dark:from-blue-900/20 dark:to-indigo-900/10 dark:text-white;
}

.markdown-body tbody tr {
  @apply transition-colors hover:bg-accent-50 dark:hover:bg-dark-700/30;
}

.markdown-body img {
  @apply my-5 max-w-full rounded-xl border border-accent-200 shadow-md dark:border-dark-600;
}

.markdown-body strong {
  @apply font-semibold text-accent-900 dark:text-white;
}

.markdown-body em {
  @apply italic text-accent-600 dark:text-accent-400;
}
</style>

<template>
  <div class="flex items-start gap-4">
    ...
    <div class="flex-shrink-0">
      <div
        class="flex items-center justify-center overflow-hidden rounded-xl border-2 border-dashed border-accent-300 bg-accent-50 dark:border-dark-600 dark:bg-dark-800"
        :class="[previewSizeClass, { 'border-solid': !!modelValue }]"
      >
        <!-- SVG mode: render inline -->
        <span
          v-if="mode === 'svg' && modelValue"
          class="text-accent-600 dark:text-accent-300 [&>svg]:h-full [&>svg]:w-full"
        ...
        <svg
          v-else
          class="text-accent-400 dark:text-dark-500"
    ...
    <div class="flex-1 space-y-2">
      ...
      <p v-if="hint" class="text-xs text-accent-500 dark:text-accent-400">{{ hint }}</p>

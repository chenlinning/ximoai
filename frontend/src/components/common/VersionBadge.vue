  ...
  <div class="relative">
    ...
    <template v-if="isAdmin">
      <button
        ...
        :class="[
          hasUpdate
            ...
            : 'bg-accent-100 text-accent-600 hover:bg-accent-200 dark:bg-dark-800 dark:text-dark-400 dark:hover:bg-dark-700'
        ]"
      ...
      >
        ...
        <span
          v-else
          class="h-3 w-12 animate-pulse rounded bg-accent-200 font-medium dark:bg-dark-600"
        ></span>
        <!-- Update indicator -->
      ...
      <transition name="dropdown">
        <div
          v-if="dropdownOpen"
          ref="dropdownRef"
          class="absolute left-0 z-50 mt-2 w-64 overflow-hidden rounded-xl border border-accent-200 bg-white shadow-lg dark:border-dark-700 dark:bg-dark-800"
        >
          <!-- Header with refresh button -->
          <div
            class="flex items-center justify-between border-b border-accent-100 px-4 py-3 dark:border-dark-700"
          >
            <span class="text-sm font-medium text-accent-700 dark:text-dark-300">{{
              t('version.currentVersion')
            }}</span>
            <button
              @click="refreshVersion(true)"
              class="rounded-lg p-1.5 text-accent-400 transition-colors hover:bg-accent-100 hover:text-accent-600 dark:hover:bg-dark-700 dark:hover:text-dark-200"
              :disabled="loading"
            ...
            <template v-else>
              ...
              <div class="mb-4 text-center">
                <div class="inline-flex items-center gap-2">
                  <span
                    v-if="currentVersion"
                    class="text-2xl font-bold text-accent-900 dark:text-white"
                    >v{{ currentVersion }}</span
                  >
                  <span v-else class="text-2xl font-bold text-accent-400 dark:text-dark-500">--</span>
                ...
                <p class="mt-1 text-xs text-accent-500 dark:text-dark-400">
                  {{
                    hasUpdate
              ...
              <div v-else-if="hasUpdate && isReleaseBuild" class="space-y-2">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  class="flex items-center justify-center gap-1 text-xs text-accent-500 transition-colors hover:text-accent-700 dark:text-dark-400 dark:hover:text-dark-200"
                >
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                class="flex items-center justify-center gap-2 py-2 text-sm text-accent-500 transition-colors hover:text-accent-700 dark:text-dark-400 dark:hover:text-dark-200"
              >
    ...
    <span v-else-if="version" class="text-xs text-accent-500 dark:text-dark-400">
      v{{ version }}
    </span>
  </div>
</template>

<template>
  <div class="relative" ref="dropdownRef">
    <button
      ...
      class="inline-flex items-center gap-1.5 rounded-lg border border-accent-200 bg-white px-2.5 py-1.5 text-xs font-medium text-accent-700 shadow-sm transition-colors hover:bg-accent-50 dark:border-dark-600 dark:bg-dark-800 dark:text-accent-300 dark:hover:bg-dark-700"
    ...
    <div
      v-if="showDropdown"
      class="absolute right-0 z-20 mt-1 w-44 rounded-lg border border-accent-200 bg-white shadow-lg dark:border-dark-600 dark:bg-dark-800"
    >
      <div class="p-1.5">
        <button
          @click="$emit('update:enabled', !enabled)"
          class="flex w-full items-center justify-between rounded-md px-3 py-1.5 text-sm text-accent-700 hover:bg-accent-100 dark:text-accent-200 dark:hover:bg-accent-700"
        >

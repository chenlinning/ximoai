<template>
  <div v-if="!isDesktopViewport" class="space-y-3">
    <template v-if="loading">
      <div v-for="i in 5" :key="i" class="rounded-lg border border-accent-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-900">
        <div class="space-y-3">
          <div v-for="column in dataColumns" :key="column.key" class="flex justify-between">
            <div class="h-4 w-20 animate-pulse rounded bg-accent-200 dark:bg-dark-700"></div>
            <div class="h-4 w-32 animate-pulse rounded bg-accent-200 dark:bg-dark-700"></div>
          </div>
          <div v-if="hasActionsColumn" class="border-t border-accent-200 pt-3 dark:border-dark-700">
            <div class="h-8 w-full animate-pulse rounded bg-accent-200 dark:bg-dark-700"></div>
          </div>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="!data || data.length === 0">
      <div class="rounded-lg border border-accent-200 bg-white p-12 text-center dark:border-dark-700 dark:bg-dark-900">
        <slot name="empty">
          <div class="flex flex-col items-center">
            <Icon
              name="inbox"
              size="xl"
              class="mb-4 h-12 w-12 text-accent-400 dark:text-dark-500"
            />
    ...
    <template v-else>
      <div
        ...
        class="rounded-lg border border-accent-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-900"
      >
        <div class="space-y-3">
          <div
          ...
          >
            <span class="text-xs font-medium uppercase tracking-wider text-accent-500 dark:text-dark-400">
              {{ column.label }}
            </span>
            <div class="text-right text-sm text-accent-900 dark:text-accent-100">
  ...
  >
    <table class="w-full min-w-max divide-y divide-accent-200 dark:divide-dark-700">
      <thead class="table-header bg-accent-50 dark:bg-dark-800">
        <tr>
          <th
            ...
            scope="col"
            :class="[
              'sticky-header-cell py-3 text-left text-xs font-medium uppercase tracking-wider text-accent-500 dark:text-dark-400',
              getAdaptivePaddingClass(),
              { 'cursor-pointer hover:bg-accent-100 dark:hover:bg-dark-700': column.sortable },
          ...
          >
            ...
            >
              <div class="flex items-center space-x-1">
                ...
                <span v-if="column.sortable" class="text-accent-400 dark:text-dark-500">
                  <svg
      ...
      <tbody class="table-body divide-y divide-accent-200 bg-white dark:divide-dark-700 dark:bg-dark-900">
        <!-- Loading skeleton -->
        <tr v-if="loading" v-for="i in 5" :key="i">
          <td v-for="column in columns" :key="column.key" :class="['whitespace-nowrap py-4', getAdaptivePaddingClass()]">
            <div class="animate-pulse">
              <div class="h-4 w-3/4 rounded bg-accent-200 dark:bg-dark-700"></div>
            </div>
          </td>
        </tr>
        ...
        <tr v-else-if="!data || data.length === 0">
          <td
            ...
            :class="['py-12 text-center text-accent-500 dark:text-dark-400', getAdaptivePaddingClass()]"
          >
            <slot name="empty">
              <div class="flex flex-col items-center">
                <Icon
        ...
        <template v-else>
          ...
          <tr
            ...
            :ref="measureElement"
            class="hover:bg-accent-50 dark:hover:bg-dark-800"
          >
            <td
              v-for="(column, colIndex) in columns"
              :key="column.key"
              :class="[
                'whitespace-nowrap py-4 text-sm text-accent-900 dark:text-accent-100',
                getAdaptivePaddingClass(),
...
.table-wrapper .table-header {
  ...
  top: 0;
  z-index: 200;
  background-color: rgb(250 249 246); /* accent-50 #FAF9F6 */
}

.dark .table-wrapper .table-header {
  background-color: rgb(44 44 44); /* dark-800 #2C2C2C */
}
...
.dark tbody .sticky-col {
  background-color: rgb(35 35 35); /* dark-900 #232323 */
}

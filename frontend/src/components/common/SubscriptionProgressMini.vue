<template>
  <div v-if="hasActiveSubscriptions" class="relative" ref="containerRef">
    ...
    <transition name="dropdown">
      <div
        v-if="tooltipOpen"
        class="absolute right-0 z-50 mt-2 w-[340px] overflow-hidden rounded-xl border border-accent-200 bg-white shadow-xl dark:border-dark-700 dark:bg-dark-800"
      >
        <div class="border-b border-accent-100 p-3 dark:border-dark-700">
          <h3 class="text-sm font-semibold text-accent-900 dark:text-white">
            {{ t('subscriptionProgress.title') }}
          </h3>
          <p class="mt-0.5 text-xs text-accent-500 dark:text-dark-400">
        ...
        <div class="max-h-64 overflow-y-auto">
          <div
            ...
            :key="subscription.id"
            class="border-b border-accent-50 p-3 last:border-b-0 dark:border-dark-700/50"
          >
            ...
            <div class="space-y-1.5">
              ...
              <template v-else>
                <div v-if="subscription.group?.daily_limit_usd" class="flex items-center gap-2">
                  <span class="w-8 flex-shrink-0 text-[10px] text-accent-500">{{
                    t('subscriptionProgress.daily')
                  }}</span>
                  <div class="h-1.5 min-w-0 flex-1 rounded-full bg-accent-200 dark:bg-dark-600">
                    <div
                  ...
                  <span class="w-24 flex-shrink-0 text-right text-[10px] text-accent-500">
                    {{
        ...
        <div class="border-t border-accent-100 p-2 dark:border-dark-700">
          <router-link
            to="/subscriptions"
            @click="closeTooltip"
...
function getDaysRemainingClass(expiresAt: string): string {
  ...
  return 'text-accent-500 dark:text-dark-400'
}
